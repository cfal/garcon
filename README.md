...
The Compose setup builds the current checkout and includes Claude Code, Codex, Cursor Agent, OpenCode, Amp, Factory Droid, Pi, Git, SSH, and the GitHub CLI. Create a `.env` file next to `docker-compose.yml` with the project directory and the non-root UID and GID that own those files. Use `id -u` and `id -g` to find the values:
...
The UID and GID must not be `0`. `GARCON_PROJECT_DIR` must be an absolute path to an existing directory owned by those IDs; change a root-owned project directory to a non-root owner before mounting it. Keep these values stable because existing named volumes retain their numeric ownership. Build and start the container with:
...
Garcon, every coding agent, terminal command, Git operation, and pull request command runs inside the container. Host project toolchains are not visible there. Add any required language runtimes or system packages to a derived image or to the runtime stage in `Dockerfile`.

Compose uses named volumes for agent credentials, configuration, and native history. This avoids host/container UID conflicts, platform-specific binaries overwriting Linux binaries, and SQLite databases on desktop bind mounts. Normal builds may reuse cached agent-installer layers; run `docker compose build --no-cache` before `docker compose up -d` when refreshing those CLIs. Agent binaries remain outside the state volumes, so rebuilding the image does not delete credentials or history. `docker compose down` preserves the volumes; `docker compose down -v` permanently deletes them.
...
Claude and Codex login can be started from Settings. Configure the other CLIs in a container shell, or provide their supported API-key environment variables to Compose:
