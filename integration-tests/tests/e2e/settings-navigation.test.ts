...
])('a malformed saved generation executor stays repairable: %j', async ({ key, label, preferences }) => {
  await withE2eFixture('settings-unavailable-generation', async (fixture) => {
    const selection = { executorId: 'not-a-executor', ...preferences };
    ...
    await fixture.page.waitForFunction(() => !document.querySelector('[role="dialog"]')?.textContent?.includes('Unavailable executor'));
    ...
    expect(repaired.uiEffective[key]?.executorId).toBe('local');
...
test('a generation toggle cannot choose an agent for an incomplete saved remote target', async () => {
  await withE2eFixture('settings-incomplete-generation', async (fixture) => {
    const selection = { executorId: fixture.integration.client.executorId, model: 'synthetic-model', enabled: true };
    await fixture.integration.restartGarcon({ beforeStart: async () => {
      const path = join(fixture.integration.dirs.workspace, 'project-settings.json');
      const stored = JSON.parse(await readFile(path, 'utf8'));
    ...
    await app.waitForText('An explicit executor requires an agent and model');
...
test('an unrelated executor update preserves a Local OAuth code', async () => {
  await withE2eFixture('settings-executor-auth-lifetime', async (fixture) => {
    await fixture.page.evaluateOnNewDocument(() => {
      ...
      Object.defineProperty(globalThis, 'fetch', { configurable: true, writable: true,
        value: (input: RequestInfo | URL, init?: RequestInit) => {
          ...
          const localClaude = url.searchParams.get('executorId') === 'local' && url.searchParams.get('agent') === 'claude';
    ...
    await fixture.integration.client.patch(`/api/v1/executors/${fixture.integration.client.executorId}`, { enabled: false });
