...
test('provider settings test and fetch models through the selected executor', async () => {
  const endpoint = Bun.serve({
    hostname: '0.0.0.0', port: 0,
    fetch: () => Response.json({ data: [{ id: 'synthetic-discovered-model' }] }),
  });
  try {
    await withE2eFixture('executor-provider-discovery', async (fixture) => {
      const { client } = fixture.integration;
      await client.post(`/api/v1/api-providers?executorId=${client.executorId}`, {
      ...
      await fixture.page.evaluateOnNewDocument(() => {
        ...
        const observe = (input: RequestInfo | URL, init?: RequestInit) => {
          ...
          if (url.pathname === '/api/v1/models') {
            catalogs.push(url.searchParams.get('executorId') ?? 'local');
            document.documentElement.dataset.catalogExecutors = JSON.stringify(catalogs);
          }
          if (['/api/v1/api-providers/test', '/api/v1/api-providers/models'].includes(url.pathname)) {
            requests.push(url.searchParams.get('executorId') ?? 'missing');
            document.documentElement.dataset.probeExecutors = JSON.stringify(requests);
          }
      ...
      await fixture.page.waitForSelector('#api-provider-executor');
      expect(await fixture.page.$eval('#api-provider-executor', (element) => (element as HTMLSelectElement).value)).toBe(client.executorId);
      ...
      expect(await fixture.page.evaluate(() => JSON.parse(document.documentElement.dataset.probeExecutors ?? '[]'))).toEqual([client.executorId, client.executorId]);
      const localCatalogRequests = await fixture.page.evaluate(() => (JSON.parse(document.documentElement.dataset.catalogExecutors ?? '[]') as string[]).filter((executorId) => executorId === 'local').length);
      ...
      await fixture.page.waitForFunction((previous) => (JSON.parse(document.documentElement.dataset.catalogExecutors ?? '[]') as string[]).filter((executorId) => executorId === 'local').length > previous, {}, localCatalogRequests);
