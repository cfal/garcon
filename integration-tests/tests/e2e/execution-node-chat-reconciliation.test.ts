...
test.each(['stay', 'chat switch', 'reload'])('pending agent choices reset on leaving but preserve composer text (%s)', async (leave) => {
  await withE2eFixture('execution-node-chat-reconciliation', async fixture => {
    ...
    await client.put(`/api/v1/api-provider-assignments?nodeId=local&apiProviderId=${directAgents.anthropic.provider.providerId}`, {});
    ...
    await fixture.page.evaluateOnNewDocument(nodeId => {
      ...
      Object.defineProperty(globalThis, 'fetch', { configurable: true, writable: true,
        value: async (input: RequestInfo | URL, init?: RequestInit) => {
          ...
          if (url.pathname !== '/api/v1/execution-nodes' || !response.ok) return response;
          const snapshot = await response.json() as { nodes: ExecutionNodeSnapshot[] };
          snapshot.nodes = snapshot.nodes.map(node => node.id === nodeId
            ? { ...node, machineServices: { files: false, git: false, gh: false, terminals: false } }
            : node);
    ...
    const moved = await client.handoffDirectChat({
      chatId, nodeId: client.nodeId, projectPath: executionDirs.project,
    ...
    const followup = 'Synthetic same-node agent change';
...
test.each(['different agent', 'durable owner'])(
  ...
  await withE2eFixture('execution-node-return-to-chat-folder', async fixture => {
    const { client, directAgents, dirs, executionDirs } = fixture.integration;
    ...
    await app.waitForButton('Integration Anthropic Echo');
    await app.clickButton('Integration Anthropic Echo');
    const picker = '[data-slot="composer-bottom-bar"] [data-execution-node-picker]';
    await selectExecutionNode(fixture.page, picker, 'Integration worker');
    await app.waitForText('Move to Integration worker');
    await app.fill('[role="dialog"] input', executionDirs.project);
    await app.waitForDialogButtonEnabled('Use This Node');
    await app.clickDialogButton('Use This Node');
    await app.waitForButton('Execution node: Integration worker');
    await selectExecutionNode(fixture.page, picker, 'Local');
    ...
    expect(mentionTarget.searchParams.get('nodeId')).toBe('local');
...
test('remote slash forks use the remote catalog when Local has no agents', async () => {
  await withE2eFixture('execution-node-remote-slash-fork', async fixture => {
    ...
    await fixture.page.evaluateOnNewDocument(() => {
      ...
      Object.defineProperty(globalThis, 'fetch', { configurable: true, writable: true,
        value: (input: RequestInfo | URL, init?: RequestInit) => {
          const url = new URL(input instanceof Request ? input.url : String(input), location.href);
          if (url.pathname === '/api/v1/models' && (url.searchParams.get('nodeId') ?? 'local') === 'local') {
    ...
    expect((await client.getChatSnapshot(forkId)).chat.nodeId).toBe(client.nodeId);
    fixture.assertNoBrowserErrors();
  }, { executionBackend: 'remote-controller-dials', projectRoots: 'separate' });
}, 90_000);
