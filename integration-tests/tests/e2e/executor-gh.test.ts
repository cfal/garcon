...
test('remote GitHub capability, PR list and diff reach the browser with their serving scope', async () => {
  await withE2eFixture('executor-gh-browser', async fixture => {
    ...
    for (const endpoint of ['status', 'pull-requests', 'pull-request']) {
      expect(requests.some(url => url.pathname === `/api/v1/gh/${endpoint}` && url.searchParams.get('executorId') === client.executorId)).toBe(true);
    }
    expect(requests.every(url => url.searchParams.get('executorId') === client.executorId)).toBe(true);
