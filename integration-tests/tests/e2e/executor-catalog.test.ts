...
test('cold remote chats load their catalog before submission and refresh it after executor replacement', async () => {
  await withE2eFixture('executor-cold-catalog', async (fixture) => {
    ...
    await fixture.page.evaluateOnNewDocument((executorId) => {
      ...
      const gatedFetch = async (input: RequestInfo | URL, init?: RequestInit) => {
        ...
        if (url.pathname === '/api/v1/models' && url.searchParams.get('executorId') === executorId) {
...
test('editing a remote schedule preserves its saved endpoint across failed discovery and Retry', async () => {
  await withE2eFixture('executor-saved-schedule-catalog', async (fixture) => {
    const { client, executionDirs, directAgents } = fixture.integration;
    const provider = directAgents.anthropic.provider;
    const target = {
      type: 'new-chat' as const, executorId: client.executorId, agentId: 'claude',
...
test('late remote catalog discovery preserves a schedule retargeted to Local', async () => {
  await withE2eFixture('executor-schedule-retarget', async (fixture) => {
    ...
    await selectExecutor(fixture.page, '[role="dialog"] [data-executor-picker]', 'Local');
    ...
    expect(saved?.type === 'new-chat' && (saved.executorId ?? 'local')).toBe('local');
    fixture.assertNoBrowserErrors();
  }, { executionBackend: 'remote-controller-dials', projectRoots: 'separate' });
}, 90_000);

test('new and scheduled chats retain input and require Retry after cached remote catalog discovery fails', async () => {
  await withE2eFixture('executor-draft-catalog', async (fixture) => {
    const { client, dirs } = fixture.integration;
    await fixture.page.evaluateOnNewDocument((executorId) => {
      const originalFetch = globalThis.fetch.bind(globalThis);
      const guardedFetch = (input: RequestInfo | URL, init?: RequestInit) => {
        const url = new URL(input instanceof Request ? input.url : String(input), location.href);
        if (url.pathname === '/api/v1/models' && url.searchParams.get('executorId') === executorId && document.documentElement.dataset.rejectRemoteCatalog === 'true') {
          return Promise.resolve(new Response('{}', { status: 503, headers: { 'content-type': 'application/json' } }));
        }
        return originalFetch(input, init);
      };
    ...
    for (const kind of ['new', 'scheduled'] as const) {
      ...
      if (kind === 'new') {
        ...
        await fixture.page.waitForFunction(() => document.querySelector('#scheduled-project-path') === null);
        expect((await client.getScheduledPrompts()).prompts[0]?.target).toMatchObject({ executorId: client.executorId });
      }
    }
    fixture.assertNoBrowserErrors();
  }, { executionBackend: 'remote-controller-dials' });
