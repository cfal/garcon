...
async function openChatWorkspace(
  ...
  await fixture.page
    .locator("[data-floating-workspace-toolbar]")
...
async function openCompare(page: Page): Promise<void> {
  ...
  await page
    .locator(`${COMPARE_PANEL} [data-git-comparison-header-row]`)
    .waitFor();
}
  ...
  await page
    ...
    .evaluate((row, value) => {
      ...
      if (!(header instanceof HTMLElement))
        throw new Error("Missing Git comparison header.");
...
async function comparisonHeaderGeometry(page: Page) {
  return page
    ...
    .evaluate((row) => {
      ...
      const range = row.querySelector<HTMLElement>(
        "[data-git-comparison-range]",
      );
      const rangeLabel = row.querySelector<HTMLElement>(
        "[data-git-comparison-range-label]",
      );
      const stats = row.querySelector<HTMLElement>(
        "[data-git-comparison-stats]",
      );
      const primaryStat = row.querySelector<HTMLElement>(
        "[data-git-comparison-primary-stat]",
      );
      const separator = row.querySelector<HTMLElement>(
        "[data-git-comparison-separator]",
      );
      const actions = row.querySelector<HTMLElement>(
        "[data-git-comparison-actions]",
      );
      if (
      ...
      const textStyles = Array.from(
        summary.querySelectorAll<HTMLElement>(
          "span:not([data-git-comparison-separator])",
        ),
        (element) => {
      ...
      return {
        ...
        separatorGapDelta: Math.abs(
          separatorRect.left -
            rangeRect.right -
            (statsRect.left - separatorRect.right),
        ),
