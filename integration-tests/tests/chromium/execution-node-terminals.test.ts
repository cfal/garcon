...
`);
    ...
    const inventory = await integration.client.get<TerminalListResponse>('/api/v1/terminals?nodeId=local');
    ...
    expect((await integration.client.get<TerminalListResponse>('/api/v1/terminals?nodeId=local')).terminals[0]?.terminalId).toBe(terminal.terminalId);
...
test('remote terminals keep their renderer and job through host reconnect and responsive moves', async () => {
  await withChromiumFixture('execution-node-terminals', async ({ page, integration, browserErrors, assertNoBrowserErrors }, markPhase) => {
    ...
    await browserExpect(page.locator('[data-composer]').getByRole('button', { name: 'Execution node: Integration worker', exact: true })).toBeVisible();
    await clickWorkspaceWindowAddAction(page, 'New Terminal');
