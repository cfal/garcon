...
test('edits remote files and retains offline buffers without touching controller files', async () => {
  await withChromiumFixture('executor-files', async ({ page, integration, assertNoBrowserErrors }) => {
    ...
    expect(new URL(response.url()).searchParams.get('executorId')).toBe(client.executorId);
    ...
    await client.waitForEvent(
      (event): event is Extract<ServerWsMessage, { type: 'executors-changed' }> => event.type === 'executors-changed' && event.executors.some((executor) => executor.id === client.executorId && executor.availability === 'ready'),
      'executor ready after re-enabling',
...
test('switches file executors from breadcrumbs without changing chat ownership and reveals complete paths', async () => {
  await withChromiumFixture('executor-file-breadcrumbs', async ({ page, integration, browserErrors }) => {
    const { client, executionDirs, dirs, directAgents } = integration;
    ...
    const chatId = integration.newChatId();
    const started = await client.startDirectChat({ chatId, content: 'Synthetic file executor navigation', projectPath: nested, agent: directAgents.openAi });
    await client.waitForTurnTerminal(chatId, started.turnId);
    ...
    const picker = breadcrumbs.locator('[data-executor-picker]');
    ...
    await picker.click();
    await page.screenshot({ path: join(dirs.root, 'file-executor-picker-desktop.png') });
    ...
    expect((await client.getChatSnapshot(chatId)).chat).toMatchObject({ executorId: client.executorId, projectPath: nested });
    ...
    await page.keyboard.press('Escape');
    await page.screenshot({ path: join(dirs.root, 'file-executor-breadcrumbs-desktop.png') });
    await page.getByRole('tab', { name: 'Files', exact: true }).click();
    await page.setViewportSize({ width: 390, height: 844 });
    await picker.click();
    await page.screenshot({ path: join(dirs.root, 'file-executor-picker-mobile.png') });
    await page.getByRole('menuitemradio', { name: 'Local', exact: true }).click();
    await browserExpect(page.locator('[data-file-tree-entry-text]').filter({ hasText: 'local-only.txt' })).toBeVisible();
    await picker.click();
    await page.getByRole('menuitemradio', { name: 'Integration worker', exact: true }).click();
    ...
    const expectedProbes = [
      { executorId: 'local', path: nested },
      { executorId: 'local', path: nested },
      { executorId: client.executorId, path: dirs.project },
      { executorId: 'local', path: nested },
      { executorId: client.executorId, path: dirs.project },
    ];
