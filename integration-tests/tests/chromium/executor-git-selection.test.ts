...
async function selectExecutor(page: Page, panel: Locator, label: string): Promise<void> {
  await panel.locator('[data-executor-picker]').click();
  await page.getByRole('menuitemradio', { name: label, exact: true }).click();
  await browserExpect(panel.locator('[data-executor-picker]')).toHaveText(label);
}
...
test('independent Git folder and executor selection survives chat changes and owns Commit and PR requests', async () => {
  await withChromiumFixture('git-independent-executor-selection', async ({ page, integration, assertNoBrowserErrors }, phase) => {
    ...
    for (const [chatId, executorId, projectPath] of [[localChat, 'local', dirs.project], [remoteChat, client.executorId, executionDirs.project]]) {
      const started = await client.startChat({ ...client.directStartRequest({ chatId, projectPath, content: 'Synthetic selection chat', agent: directAgents.openAi }), executorId });
    ...
    await selectExecutor(page, git, 'Integration worker');
    ...
    await browserExpect(commit.locator('[data-executor-picker]')).toHaveText('Integration worker');
    ...
    expect(response.request().postDataJSON()).toMatchObject({ executorId: client.executorId, project: alternate });
    ...
    phase('PR capability and requests follow its own selected executor');
    ...
    await selectExecutor(page, pr, 'Integration worker');
    ...
    await browserExpect(git.locator('[data-executor-picker]')).toHaveText('Local');
    await git.getByText('Local project change', { exact: false }).first().waitFor();
