...
test('chat host selectors fit narrow containers and stage complete cancellable handoffs', async () => {
  ...
  await withChromiumFixture('chat-host-selection', async ({ page, integration, browserErrors }, phase) => {
    ...
    const host = () => controls().getByRole('button', { name: `Executor: ${label}`, exact: true });
    ...
    await dialog.locator('[data-executor-picker]').click();
    ...
    for (const width of [1440, 390]) {
      ...
      expect(await picker.locator('.executor-label').evaluate(element => getComputedStyle(element).display)).not.toBe('none');
    ...
    expect(await page.locator('[data-slot="model-selector-executors"]').count()).toBe(0);
    await page.keyboard.press('Escape');
    await page.keyboard.press('Escape');
    ...
    await browserExpect(columns.locator('[data-slot="model-selector-executors"]')).toBeVisible();
    expect(await columns.evaluate(element => Boolean(element.firstElementChild?.querySelector('[data-slot="model-selector-executors"]')))).toBe(true);
    ...
    for (const failure of failures) {
      ...
      expect(Object.fromEntries(url.searchParams)).toEqual({
        chatId, expectedProjectPath: executionDirs.project, executorId: client.executorId,
      });
...
test('accepted executor handoffs update the Files target before background chat refresh', async () => {
  await withChromiumFixture('chat-host-handoff-project', async ({ page, integration, assertNoBrowserErrors }) => {
    ...
    const controls = page.locator('[data-slot="composer-bottom-bar"]:visible');
    await controls.getByRole('button', { name: 'Executor: Integration worker', exact: true }).click();
    await page.getByRole('menuitemradio', { name: 'Local', exact: true }).click();
    await page.getByLabel('Destination project folder').fill(integration.dirs.project);
    await page.getByRole('button', { name: 'Use This Executor', exact: true }).click();
    await browserExpect(controls.getByRole('button', { name: 'Executor: Local', exact: true })).toBeVisible();
