...
describe('prompt refinement', () => {
  test.each(['prompt', 'issue-description'] as const)('refines %s using the saved target and complete template', async (refinementTarget) => {
    ...
    await withIntegrationFixture(
      ...
      async (fixture) => {
        ...
        const expectedModelPrompt = refinementTarget === 'issue-description'
