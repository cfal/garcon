...
for (const executionBackend of ['remote-controller-dials', 'remote-node-dials'] as const) {
  test(`ticket defaults use global repository names on the owning node (${executionBackend})`, async () => {
    await withIntegrationFixture(`ticket-defaults-${executionBackend}`, async (f) => {
      const resolve = (directory: string, nodeId: string) =>
        f.client.post('/api/v1/tickets/project-default', { directory, nodeId });
      for (const [root, nodeId] of [[f.dirs.project, 'local'], [f.executionDirs.project, f.client.nodeId]]) {
        ...
        for (const directory of [repo, linked, join(linked, 'nested')]) {
          expect(await resolve(directory, nodeId!)).toEqual({ project: 'shared-repo', kind: 'repository' });
        }
        const folder = join(root!, 'plain');
        await mkdir(folder);
        expect(await resolve(folder, nodeId!)).toEqual({ project: 'plain', kind: 'folder' });
      }
      await expect(resolve(f.dirs.project, f.client.nodeId))
