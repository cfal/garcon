  ...
  for (const restart of [false, true]) {
    test(`offline native deletion retries ${restart ? 'after restart' : 'on reconnect'} (${executionBackend})`, async () => {
      await withIntegrationFixture(`native-delete-${executionBackend}-${restart}`, async (fixture) => {
        ...
        await fixture.client.patch(`/api/v1/executors/${executorId}`, { enabled: false });
        ...
        if (restart) {
          await fixture.restartGarcon({ beforeStart: async () => {
            const path = join(fixture.dirs.workspace, 'executors.json');
            const config = JSON.parse(await readFile(path, 'utf8'));
            const executor = config.executors.find((entry: { id: string }) => entry.id === executorId);
            executor.enabled = true;
            await writeFile(path, JSON.stringify(config));
          } });
        } else {
          await fixture.client.patch(`/api/v1/executors/${executorId}`, { enabled: true });
        }
