...
for (const backend of ['remote-controller-dials', 'remote-executor-dials'] as const) {
  test(`oversized provider output fails its chat without disconnecting another (${backend})`, async () => {
    await withIntegrationFixture(`oversized-output-${backend}`, async (fixture) => {
      ...
      expect(parseExecutors(snapshot.executors)?.find(executor => executor.id === client.executorId)?.availability).toBe('ready');
  ...
  test(`a producer ledger failure leaves other chats on the same executor running (${backend})`, async () => {
  ...
  test(`oversized chat input does not interrupt another chat on the executor (${backend})`, async () => {
