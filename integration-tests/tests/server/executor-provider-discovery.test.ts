...
test.each(['remote-controller-dials', 'remote-executor-dials'] as const)('endpoint probes use the selected %s executor without falling back when disabled', async (executionBackend) => {
  ...
  try {
    await withIntegrationFixture(`provider-discovery-${executionBackend}`, async ({ client }) => {
      ...
      const saved = await client.post<ApiProviderCatalogEntry>(`/api/v1/api-providers?executorId=${client.executorId}`, input);
      ...
      for (const [path, body] of [['test', input], ['models', discovery]] as const) {
        const result = await client.post<ApiProviderModelDiscoveryResponse>(`/api/v1/api-providers/${path}?executorId=${client.executorId}`, body);
