...
async function runtimeFile(fixture: IntegrationFixture): Promise<string> {
  const file = cliRuntimeFile(fixture.executionDirs.config, 'executor');
...
for (const backend of ['remote-controller-dials', 'remote-executor-dials'] as const) {
  test(`CLI gateway preserves targeting, executor authority and a retained shell across restart (${backend})`, async () => {
    await withIntegrationFixture(`execution-cli-${backend}`, async (fixture) => {
      ...
      try {
        ...
        const continued = await runCli(fixture, ['resume', localId, '--model', localAgent.provider.model, 'Synthetic cross-executor continuation.'], 'executor');
        expect(continued, continued.stderr).toMatchObject({ exitCode: 0 });
        expect(effectiveExecutorId((await local.getChatSnapshot(localId)).chat.executorId)).toBe('local');
      } finally { await local.close(); }
      const created = await runCli(fixture, ['ticket', 'create', '--title', 'Synthetic executor ticket', '--json']);
      expect(created.exitCode).toBe(0);
      expect(created.stderr).toContain("--runtime $'executor'");
      ...
      expect(ticket.ticket).toMatchObject({ project: basename(fixture.executionDirs.project), createdBy: { kind: 'executor', executorId } });
      const claim = await runCli(fixture, ['ticket', 'claim', ticket.ticket.id, '--expected-revision', String(ticket.ticket.revision), '--json'], 'executor');
      expect(claim, claim.stderr).toMatchObject({ exitCode: 0 });
      expect(parseTicketWriteResult(JSON.parse(claim.stdout)).ticket.assignee).toEqual({ kind: 'executor', executorId });
      ...
      const { terminal } = await client.post<TerminalCreateResponse>('/api/v1/terminals', {
        executorId, expectedTerminalRuntimeId: inventory.terminalRuntimeId, requestId: 'cli-shell', requestedInitialWorkingDirectory: fixture.executionDirs.project,
      });
      ...
      const retry = await runCli(fixture, ['ticket', 'create', '--title', 'Synthetic executor ticket', '--project', ticket.ticket.project,
        '--request-id', retryId!, '--expected-store-id', ticket.storeId, '--json'], 'executor');
      expect(retry, retry.stderr).toMatchObject({ exitCode: 0 });
      expect(parseTicketWriteResult(JSON.parse(retry.stdout))).toEqual(ticket);
      attachment = await attach(client, terminal.terminalId, await client.get<TerminalListResponse>(`/api/v1/terminals?executorId=${executorId}`));
      expect(await shellCommand(client, terminal.terminalId, attachment, command)).toContain(`"id": "${chatId}"`);
      await client.patch(`/api/v1/executors/${executorId}`, { allowControllerCli: false });
    ...
    }, { executionBackend: backend, projectRoots: 'separate', namedWorkspace: 'cli-executor-integration',
...
test.each([true, false])('shared-root roles select by start time while children and retry prefixes retain their origin: named=%s', async (named) => {
  await withIntegrationFixture('cli-controller-child-context', async (fixture) => {
    ...
    expect((await discoverRuntime({ configDir: fixture.dirs.config }, { warn: (message) => warnings.push(message) })).defaultExecutorId).toBe(fixture.client.executorId);
    expect(warnings[0]).toContain('selected executor');
    const created = await runCli(fixture, ['ticket', 'create', '--title', 'Synthetic worker ticket', '--project', 'Synthetic project', '--json']);
    expect(created.exitCode).toBe(0);
    expect(created.stderr).toContain('both runtime files exist; selected executor');
    ...
    const inventory = await fixture.client.get<TerminalListResponse>('/api/v1/terminals?executorId=local');
    const { terminal } = await fixture.client.post<TerminalCreateResponse>('/api/v1/terminals', {
      executorId: 'local', expectedTerminalRuntimeId: inventory.terminalRuntimeId, requestId: 'controller-cli-shell',
    ...
    expect(output).not.toContain('"kind":"executor"');
    expect(output).not.toContain('both runtime files exist');
    const retarget = `${shellQuote(process.execPath)} ${shellQuote(CLI)} --config-dir ${shellQuote(fixture.executionDirs.config)} --runtime executor list agents --json`;
...
test('a real permission-approved Claude tool inherits worker CLI discovery and calls back over the same channel', async () => {
  ...
  try {
    await withIntegrationFixture('execution-cli-provider', async (fixture) => {
      ...
      await fixture.client.patch(`/api/v1/executors/${executorId}`, { allowControllerCli: true });
      ...
      expect(allow).toContain("--runtime 'executor'");
      expect(allow).toContain('--config-dir');
      const decision = await runWorkerCommand(fixture, ['/bin/sh', '-c',
        `${shellQuote(process.execPath)} ${shellQuote(CLI)}${allow!.slice('garcon-cli'.length)}`], 'executor');
    ...
    }, { executionBackend: 'remote-executor-dials', projectRoots: 'separate', serverEnvironment: environment.serverEnvironment });
...
test('a missing worker runtime file keeps execution and terminals available without falling back to Local CLI', async () => {
  await withIntegrationFixture('execution-cli-unavailable', async (fixture) => {
    ...
    expect(local.defaultExecutorId).toBe('local');
    ...
    expect((await client.getChatSnapshot(chatId)).chat.executorId).toBe(executorId);
    const inventory = await client.get<TerminalListResponse>(`/api/v1/terminals?executorId=${executorId}`);
    const { terminal } = await client.post<TerminalCreateResponse>('/api/v1/terminals', {
      executorId, expectedTerminalRuntimeId: inventory.terminalRuntimeId, requestId: 'cli-unavailable-shell',
    ...
    expect(output).toContain('no executor runtime file');
