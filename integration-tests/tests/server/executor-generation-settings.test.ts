...
test('malformed generation updates preserve durable selections and readable settings', async () => {
  await withIntegrationFixture('executor-generation-settings', async (fixture) => {
    ...
    const executor = await client.post<{ id: string }>('/api/v1/executors', {
      label: 'Offline generator', direction: 'executor-connects',
    });
    ...
    for (const key of GENERATION_UI_SETTING_KEYS) {
      ...
      expect(snapshot.uiEffective[key]?.executorId).toBe(executor.id);
    }
    ...
    for (const key of GENERATION_UI_SETTING_KEYS) {
      ...
      expect(reset.uiEffective[key]?.executorId).toBe('local');
    }
...
test('malformed persisted generation targets remain repairable through settings reads and broadcasts', async () => {
  await withIntegrationFixture('unavailable-generation-settings', async (fixture) => {
    ...
    const agent = fixture.directAgents.openAi;
    const selections = {
      chatTitle: { executorId: 'not-a-executor', agentId: agent.agentId, model: agent.provider.model, enabled: true },
      agentSwitchCompaction: { executorId: executor, enabled: true },
      commitMessage: { executorId: 42, agentId: agent.agentId, model: agent.provider.model },
      promptRefinement: { executorId: executor, model: agent.provider.model },
    };
    ...
    const check = (snapshot: RemoteSettingsSnapshot) => {
      ...
      for (const key of GENERATION_UI_SETTING_KEYS) {
        expect(parsed?.ui[key]).toMatchObject({ ...selections[key], executorId: key === 'commitMessage' ? '' : selections[key].executorId });
