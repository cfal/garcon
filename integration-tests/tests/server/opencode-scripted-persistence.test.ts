...
describeOnLinux('scripted OpenCode persistence', () => {
  ...
  test('[TLV5-FORK.03-OPENCODE-SCRIPTED-01] forks natively at a message point and seeds only the prefix', async () => {
    const testEnvironment = requireEnvironment();
    const firstPrompt = marker('POINT_FORK_FIRST_PROMPT');
    const firstReply = marker('POINT_FORK_FIRST_REPLY');
    const secondPrompt = marker('POINT_FORK_SECOND_PROMPT');
    const secondReply = marker('POINT_FORK_SECOND_REPLY');
    const forkPrompt = marker('POINT_FORK_CONTINUATION_PROMPT');
    const forkReply = marker('POINT_FORK_CONTINUATION_REPLY');
    testEnvironment.model.scriptTurn([chatCompletionsText(firstReply)]);
    testEnvironment.model.scriptTurn([chatCompletionsText(secondReply)]);
    testEnvironment.model.scriptTurn([chatCompletionsText(forkReply)]);

    await withIntegrationFixture('opencode-scripted-point-fork', async (fixture) => {
      const sourceChatId = fixture.newChatId();
      const firstCursor = fixture.client.markEvents();
      const first = await fixture.client.startChat(scriptedOpenCodeStartRequest({
        chatId: sourceChatId,
      ...
      const anchor = sourceTranscript.messages.find((entry) => (
      ...
      expect(userContents(forked.messages)).toEqual([firstPrompt]);
      expect(assistantContents(forked.messages)).toEqual([firstReply]);
      const forkNative = await openCodeNativeSession(fixture, forkChatId);
      const sourceNative = await openCodeNativeSession(fixture, sourceChatId);
      expect(forkNative.agentSessionId).not.toBe(sourceNative.agentSessionId);
      expect(readOpenCodeSessionRows(forkNative).messages).toHaveLength(2);
      expect(readOpenCodeSessionRows(sourceNative).messages).toHaveLength(4);
      ...
      expect(readOpenCodeSessionRows(sourceNative).messages).toHaveLength(4);
      expect(assistantContents((await fixture.client.getMessages(sourceChatId)).messages))
        .toEqual([firstReply, secondReply]);
      testEnvironment.model.assertSettled();
    }, withScriptedOpenCode());
  }, 120_000);

  test('[TLV5-FORK.01-OPENCODE-SCRIPTED-01] refuses a fork point missing from native history until handoff consent', async () => {
    const testEnvironment = requireEnvironment();
    const firstPrompt = marker('REFUSAL_FIRST_PROMPT');
    const firstReply = marker('REFUSAL_FIRST_REPLY');
    const secondPrompt = marker('REFUSAL_SECOND_PROMPT');
    const secondReply = marker('REFUSAL_SECOND_REPLY');
    testEnvironment.model.scriptTurn([chatCompletionsText(firstReply)]);
    testEnvironment.model.scriptTurn([chatCompletionsText(secondReply)]);

    await withIntegrationFixture('opencode-scripted-fork-refusal', async (fixture) => {
      const sourceChatId = fixture.newChatId();
      const firstCursor = fixture.client.markEvents();
      const first = await fixture.client.startChat(scriptedOpenCodeStartRequest({
        chatId: sourceChatId,
      ...
      const transcript = await fixture.client.getMessages(sourceChatId);
      const anchor = [...transcript.messages].reverse().find((entry) => (
        entry.message.type === 'assistant-message'
      ));
      if (!anchor) throw new Error('Source transcript is missing an assistant row.');

      // Removing the newest native message leaves the anchor row without a
      // provider-native position, which is exactly the not-settled shape.
      const sourceNative = await openCodeNativeSession(fixture, sourceChatId);
      const newestMessageId = readOpenCodeSessionRows(sourceNative).messages.at(-1)?.id;
      if (!newestMessageId) throw new Error('Native session has no stored messages.');
      const database = new Database(openCodePaths(fixture.dirs).database, { strict: true });
      try {
        database.transaction(() => {
          database.query('DELETE FROM part WHERE message_id = ?').run(newestMessageId);
          database.query('DELETE FROM message WHERE id = ?').run(newestMessageId);
        })();
      } finally {
        database.close();
      }
      ...
      const forked = await fixture.client.getMessages(forkChatId);
      expect(userContents(forked.messages)).toEqual([firstPrompt, secondPrompt]);
      expect(assistantContents(forked.messages)).toEqual([firstReply, secondReply]);
      testEnvironment.model.assertSettled();
    }, withScriptedOpenCode());
  }, 120_000);
