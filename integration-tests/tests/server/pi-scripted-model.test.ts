...
describe('Pi against a scripted model', () => {
  ...
  test('resumes the same native session through the publisher issued after transcript reload', async () => {
    ...
    await withIntegrationFixture('pi-scripted-reload-routing', async (fixture) => {
      const chatId = fixture.newChatId();
      const firstCursor = fixture.client.markEvents();
      const first = await fixture.client.startChat(scriptedPiStartRequest({
        chatId,
      ...
      const sessionBeforeReload = await piNativeSession(fixture, chatId);
      ...
      const sessionAfterReload = await piNativeSession(fixture, chatId);
