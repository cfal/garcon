...
test('shutdown rejects browser work while the remote abort reply is held', async () => {
  await withIntegrationFixture('execution-node-shutdown', async fixture => {
    ...
    try {
      ...
      while (true) {
        const snapshot = await fixture.client.get<{ nodes: unknown }>('/api/v1/execution-nodes');
        if (parseExecutionNodes(snapshot.nodes)?.some(entry => entry.id === node.id && entry.availability === 'ready')) break;
        if (Date.now() >= deadline) throw new Error('Shutdown worker did not become ready');
        await Bun.sleep(20);
      }
      const agent = fixture.directAgents.openAi;
      await fixture.client.put(`/api/v1/api-provider-assignments?nodeId=${node.id}&apiProviderId=${agent.provider.providerId}`, {});
      ...
      await fixture.client.startDirectChat({
        nodeId: node.id, chatId: fixture.newChatId(), projectPath: directories.project, agent,
