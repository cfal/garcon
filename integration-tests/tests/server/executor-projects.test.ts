...
import { assertRealWithinBase } from '../../../server/common/path-boundary.js';
...
for (const backend of ['remote-controller-dials', 'remote-executor-dials'] as const) {
  test(`project inputs stay on the worker with disjoint roots (${backend})`, async () => {
    await withIntegrationFixture(`remote-projects-${backend}`, async (fixture) => {
      ...
      expect(await client.get(`/api/v1/projects/resolve?executorId=${executorId}&projectPath=${encodeURIComponent(fixture.dirs.project)}`))
      ...
      const created = await client.post<PreamblesMutationResponse>('/api/v1/preambles', {
        ...
        preamble: {
          ...
          scope: { type: 'project-paths', rules: [{ executorId, projectPath, includeNested: true }] },
        },
      });
      ...
      expect(await client.get(`/api/v1/files/browse?executorId=${executorId}&path=${encodeURIComponent(projectPath)}`)).toBeArray();
      expect(await client.post('/api/v1/git/quick-summary', { executorId, project: projectPath }))
        .toMatchObject({ executorId, status: 'not-git-repository' });
      expect(await client.get(`/api/v1/gh/status?executorId=${executorId}`)).toMatchObject({ executorId });
      expect(await client.get(`/api/v1/terminals?executorId=${executorId}`)).toMatchObject({ success: true, terminals: [] });
