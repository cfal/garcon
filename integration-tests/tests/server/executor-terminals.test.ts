...
for (const backend of ['in-process', 'remote-controller-dials', 'remote-executor-dials'] as const) {
  test(`real PTYs preserve identity and jobs through browser/controller replacement (${backend})`, async () => {
    await withIntegrationFixture(`terminals-${backend}`, async fixture => {
      ...
      const inventory = await list(client, executorId);
      expect(inventory.terminalRuntimeId).toBeString();
      const request = { executorId, expectedTerminalRuntimeId: inventory.terminalRuntimeId, requestId: 'synthetic-create', requestedInitialWorkingDirectory: fixture.executionDirs.project };
      const { terminal } = await client.post<TerminalCreateResponse>('/api/v1/terminals', request);
      const terminalId = terminal.terminalId;
      expect(parseTerminalReference(terminalId)).toMatchObject({ executorId, terminalRuntimeId: inventory.terminalRuntimeId });
      ...
      const restored = await attach(client, terminalId, await list(client, executorId));
      ...
      if (backend !== 'in-process') {
        ...
        const after = await list(client, executorId);
      ...
      expect((await list(client, executorId)).terminals[0]).toMatchObject({ terminalId, exitCode: 7 });
