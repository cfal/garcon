  ...
  test(`Git admits a bounded burst of concurrent metadata reads (${executionBackend})`, async () => {
    ...
    try {
      await withIntegrationFixture(`git-admission-${executionBackend}`, async fixture => {
        ...
        const endpoint = `/api/v1/git/status?${new URLSearchParams({ executorId: client.executorId, project: executionDirs.project })}`;
        holding = true;
        ...
        for (const result of await Promise.all(pending)) expect(result).toMatchObject({ executorId: client.executorId, branch: 'main' });
        expect(await client.get(endpoint)).toMatchObject({ executorId: client.executorId, branch: 'main' });
      }, {
