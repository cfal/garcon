...
for (const backend of ['remote-controller-dials', 'remote-node-dials'] as const) {
  test(`worker base changes refresh metadata and enforce new boundaries without controller restart (${backend})`, async () => {
    await withIntegrationFixture(`project-base-${backend}`, async (fixture) => {
      ...
      const route = `/api/v1/files/text?${new URLSearchParams({ nodeId, projectPath: originalPath, path: 'file.txt' })}`;
      ...
      const local = await client.startDirectChat({ nodeId: 'local', chatId: localChatId, projectPath: dirs.project, content: 'Synthetic unaffected local turn', agent: fixture.directAgents.openAi });
      await held.received;
      const snapshot = async () => (await client.get<{ nodes: ExecutionNodeSnapshot[] }>('/api/v1/execution-nodes')).nodes.find((node) => node.id === nodeId)!;
      let previous = await snapshot();
      const originalLocal = (await client.get<{ nodes: ExecutionNodeSnapshot[] }>('/api/v1/execution-nodes')).nodes[0];
      for (const base of ['/', narrow]) {
        const eventIndex = client.eventRecords().length;
        await fixture.crashAndRestartExecutionWorker(base);
        ...
        await client.waitForEvent(
          (event): event is Extract<ServerWsMessage, { type: 'execution-nodes-changed' }> => event.type === 'execution-nodes-changed' && event.nodes.some((node) => node.id === nodeId && node.instanceId === current.instanceId && node.projectBasePath === base),
          'accepted replacement metadata', { afterIndex: eventIndex },
        );
        expect((await client.getChatSnapshot(chatId)).chat).toMatchObject({ nodeId, projectPath: originalPath });
        expect(fixture.garcon.pid).toBe(pid);
        const nodes = (await client.get<{ nodes: ExecutionNodeSnapshot[] }>('/api/v1/execution-nodes')).nodes;
        expect(nodes[0]).toEqual(originalLocal);
        if (base === '/') {
          expect(await client.get<ReadTextResponse>(route)).toEqual(original);
          const tree = await client.get<FileTreeResponse>(`/api/v1/files/tree?${new URLSearchParams({ nodeId, path: originalPath })}`);
          expect(tree.fileRootPath).toBe('/');
          expect(tree.directory.breadcrumbs[0]).toEqual({ name: '/', path: '/' });
          expect(await client.get(`/api/v1/projects/resolve?${new URLSearchParams({ nodeId, projectPath: dirs.project })}`))
          ...
          expect(await client.get(`/api/v1/projects/resolve?${new URLSearchParams({ nodeId, projectPath: originalPath })}`))
          ...
          const escape = `/api/v1/files/text?${new URLSearchParams({ nodeId, projectPath: narrow, path: 'escape/file.txt' })}`;
          await expect(client.get(escape)).rejects.toMatchObject({ status: 403 });
          const inner = `/api/v1/files/text?${new URLSearchParams({ nodeId, projectPath: narrow, path: 'inner.txt' })}`;
