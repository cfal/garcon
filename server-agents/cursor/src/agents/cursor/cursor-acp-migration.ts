...
interface CursorMigrationChatEntry {
  agentId: string;
...
interface CursorMigrationRegistry {
  getRegistry(): { sessions: Record<string, CursorMigrationChatEntry> };
  saveRegistry(snapshot: { sessions: Record<string, CursorMigrationChatEntry> }): Promise<void>;
}
...
export async function migrateCursorStreamJsonSessionsToAcp(
  registry: CursorMigrationRegistry,
  cursorHome?: string,
