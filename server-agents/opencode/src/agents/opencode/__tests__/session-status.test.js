...
import { openCodeRetryNoticeText, openCodeSessionRetryStatus } from '../sse-events.js';
...
function pushStatus(eventStream, sessionId, status) {
  ...
  eventStream.push({
    id: `event-status-${statusEventCounter}`,
    type: 'session.status',
...
describe('OpenCode session status', () => {
  it('publishes one titled retry notice per scheduled attempt', async () => {
    ...
    pushStatus(eventStream, 'session-1', retry);
    pushStatus(eventStream, 'session-1', retry);
    await waitFor(() => noticeEvents(events).length >= 1);

    expect(noticeEvents(events)).toEqual([{
      type: 'notice',
      runId: 'run-a',
      title: 'Provider retry',
      content: 'Model provider retrying (attempt 2): Provider is overloaded.'
        + ' Next attempt at 2026-08-21T20:23:00.000Z.',
    }]);

    pushStatus(eventStream, 'session-1', { ...retry, attempt: 3 });
    await waitFor(() => noticeEvents(events).length === 2);

    expect(noticeEvents(events)[1].content).toContain('(attempt 3)');

    eventStream.close();
    await runtime.shutdown();
  });

  it('ignores busy and idle statuses, foreign sessions, and completed turns', async () => {
    ...
    pushStatus(eventStream, 'session-1', { type: 'idle' });
...
describe('openCodeSessionRetryStatus', () => {
  it('reads the typed retry arm and rejects everything else', () => {
    expect(openCodeSessionRetryStatus({
      type: 'session.status',
      properties: {
        sessionID: 's',
  ...
  it('tolerates a missing attempt and an unusable next timestamp', () => {
    expect(openCodeSessionRetryStatus({
      type: 'session.status',
      properties: {
        sessionID: 's',
        status: { type: 'retry', message: 'quota exhausted', next: Number.NaN },
      },
    })).toEqual({ attempt: 0, message: 'quota exhausted', nextAttemptAt: null });
  });
});

describe('openCodeRetryNoticeText', () => {
  it('composes attempt and next-attempt clauses without duplicated periods', () => {
    expect(openCodeRetryNoticeText({
      attempt: 3,
      message: 'Quota exhausted. Resets soon.',
      nextAttemptAt: '2026-08-21T20:23:00.000Z',
    })).toBe(
      'Model provider retrying (attempt 3): Quota exhausted. Resets soon.'
        + ' Next attempt at 2026-08-21T20:23:00.000Z.',
    );
    expect(openCodeRetryNoticeText({
      attempt: 0,
      message: 'Provider is overloaded',
      nextAttemptAt: null,
    })).toBe('Model provider retrying: Provider is overloaded.');
  });
});
