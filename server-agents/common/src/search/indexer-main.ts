...
import { projectSearchMessage, TRANSCRIPT_SEARCH_PROJECTOR_VERSION } from './message-projector.js';
import {
  closeSearchDatabase,
  deleteChatRows,
  getChatSafetyStates,
...
import { compareGeneration, isIndexerRequest, workerRequestIdentity } from './worker-protocol.js';
  ...
  try {
    ...
    while (true) {
      ...
      if (chunk.chunkIndex !== expectedChunkIndex
          || chunk.messages.length > LOAD_LIMITS.maxMessagesPerBatch
          || Buffer.byteLength(JSON.stringify(chunk.messages)) > LOAD_LIMITS.maxBatchBytes) {
...
function validateNativeBatch(batch: readonly ChatMessage[]): ChatMessage[] {
  if (batch.length > LOAD_LIMITS.maxMessagesPerBatch) {
    throw new AgentTranscriptIndexError({
      kind: 'agent-transcript-index-failure',
  ...
  for (const raw of batch) {
    ...
    if (recordBytes > LOAD_LIMITS.maxRecordBytes || batchBytes > LOAD_LIMITS.maxBatchBytes) {
      throw new AgentTranscriptIndexError({
        kind: 'agent-transcript-index-failure',
        code: 'SOURCE_BATCH_TOO_LARGE',
        retryable: false,
...
function stripFirstUserSeedPreservingSource(batch: ChatMessage[]): ChatMessage[] {
...
function catalogEntryKey(entry: TranscriptSearchCatalogEntry): string {
...
function validateCatalogEntry(entry: TranscriptSearchCatalogEntry): void {
...
async function handle(request: IndexerRequest): Promise<void> {
