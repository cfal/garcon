...
// Amp native namespace. Amp is a shared-engine best-effort integration; this
// proves the engine never collapses equal content or mutates a committed
// envelope for it.
describe('amp projection conformance', () => {
  it('upholds the shared journal identity contract', async () => {
    await expect(runProjectionConformance({ ownerId: 'amp' })).resolves.toBeUndefined();
  });
});
