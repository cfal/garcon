...
// direct Chat Completions native namespace. The direct store already persists
// complete rendered messages; this proves the engine preserves their identity.
describe('direct-openai-compatible projection conformance', () => {
  it('upholds the shared journal identity contract', async () => {
    await expect(
      runProjectionConformance({ ownerId: 'direct-openai-compatible' }),
