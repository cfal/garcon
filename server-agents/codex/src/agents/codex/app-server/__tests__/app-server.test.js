...
describe('CodexAppServerRuntime', () => {
  ...
  it.each([
    ...
    try {
      ...
      await expect(pending).rejects.toThrow('Codex session ended before starting the turn');
  ...
  it('cancels approvals for one native turn through its captured turn operation', async () => {
    ...
    fake.emit('serverRequest', {
      id: 71,
      method: 'item/commandExecution/requestApproval',
      params: { threadId: 'thread-1', turnId: 'turn-1', itemId: 'cmd-a', command: 'command-a' },
    });
    fake.emit('serverRequest', {
      id: 72,
      method: 'item/commandExecution/requestApproval',
      params: { threadId: 'thread-1', turnId: 'turn-1', itemId: 'cmd-b', command: 'command-b' },
    });
  ...
  it('fences unexpected turns after an ambiguous settings timeout', async () => {
    const { fake, provider, published, started } = await startSettingsSession({
