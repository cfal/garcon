...
function executor(
	id: string,
	availability: ExecutorSnapshot['availability'] = 'ready',
...
function terminal(executorId: string, sequence = 1, runtime = runtimeId): TerminalMetadata {
	return {
		terminalId: `${executorId}/${runtime}/00000000-0000-4000-8000-${String(sequence).padStart(12, '0')}`,
		title: null,
...
function setup(options: { realTransport?: boolean } = {}) {
	...
	executors.applySnapshot([executor('local'), executor(remoteId)]);
	const list = vi.fn(async (executorId = 'local') => inventory([terminal(executorId)]));
	...
	return { registry, executors, list, create, sent, callbacks, renderer, write, dispose, setConnected };
}

describe('executor-qualified terminal registry', () => {
	...
	it.each(['initial connection', 'browser reconnect'])(
		...
		async (phase) => {
			...
			const attachmentCount = (executorId: string) =>
				sent.filter(
					(message) =>
						message.type === 'terminal-attach' &&
						message.terminalId === terminal(executorId).terminalId,
				).length;
			...
			try {
				...
				expect(registry.executorInventories[remoteId].status).toBe('loading');
	...
	it('restores a successful reconnect retry while another host inventory remains pending', async () => {
		vi.useFakeTimers();
		const { registry, list, sent, setConnected } = setup({ realTransport: true });
		const attachments = (executorId: string) =>
			sent.filter(
				(message) =>
					message.type === 'terminal-attach' && message.terminalId === terminal(executorId).terminalId,
			);
		await registry.initialize();
		...
		setConnected(true);
		try {
			await vi.waitFor(() => expect(registry.executorInventories.local.status).toBe('failed'));
			expect(registry.transportStatus).toBe('reconciling');
			await vi.advanceTimersByTimeAsync(5_000);
			expect(localAttempts).toBe(2);
			expect(registry.executorInventories.local.status).toBe('ready');
			expect(registry.executorInventories[remoteId].status).toBe('loading');
			expect(registry.transportStatus).toBe('connected');
	...
	it.each(['disconnect', 'logout', 'executor loss', 'destroy'])(
		'does not restore an interrupted inventory retry after %s',
		async (interruption) => {
			vi.useFakeTimers();
			const { registry, executors, list, sent, setConnected } = setup({ realTransport: true });
			...
			try {
				...
				else if (interruption === 'executor loss') {
					executors.applySnapshot([executor('local', 'offline'), executor(remoteId)]);
				...
				else executors.applySnapshot([executor('local'), executor(remoteId)]);
	...
	it('retains reconciliation backoff when every reconnect inventory fails', async () => {
		...
		await vi.waitFor(() =>
			expect(
				Object.values(registry.executorInventories).every(({ status }) => status === 'failed'),
			).toBe(true),
		);
		expect(registry.transportStatus).toBe('reconciling');
		expect(attachments()).toHaveLength(2);
	...
	it('restores confirmed attachments after logout and login clear their authority', async () => {
		...
		await vi.waitFor(() => expect(attachments()).toHaveLength(4));
		for (const executorId of ['local', remoteId]) {
			const requests = attachments().filter(
				(message) => message.terminalId === terminal(executorId).terminalId,
			);
	...
	it('keeps Local first and preserves configured remote host order', () => {
		const { registry, executors } = setup();
		const otherRemoteId = '00000000-0000-4000-8000-000000000004';
		executors.applySnapshot([executor(otherRemoteId), executor('local'), executor(remoteId, 'offline')]);
	...
	it.each(['before inventory', 'after inventory'])(
		...
		async (ordering) => {
			...
			expect(registry.executorInventories[remoteId].runtimeId).toBe(replacementRuntimeId);
	...
	it('retries a failed ready-executor inventory after partial transport reconciliation succeeds', async () => {
		vi.useFakeTimers();
		...
		await vi.waitFor(() => expect(registry.transportStatus).toBe('connected'));
		expect(registry.sessions[terminal('local').terminalId]).toBeDefined();
		expect(registry.executorInventories[remoteId].status).toBe('failed');
		const localCalls = list.mock.calls.filter(([id]) => id === 'local').length;
		remoteUnavailable = false;
		await vi.advanceTimersByTimeAsync(5_000);
		expect(registry.executorInventories[remoteId].status).toBe('ready');
	...
	it.each(['destroy', 'logout', 'offline'])('does not retry inventory after %s', async (action) => {
		vi.useFakeTimers();
		const { registry, list, executors } = setup();
		list.mockImplementation(async (id = 'local') => {
		...
		else executors.applySnapshot([executor('local'), executor(remoteId, 'offline')]);
		await vi.advanceTimersByTimeAsync(10_000);
		expect(list).toHaveBeenCalledTimes(calls);
	});

	it('resumes failed-executor inventory retries after login when successful hosts are empty', async () => {
		vi.useFakeTimers();
		const { registry, list, sent } = setup({ realTransport: true });
		...
		await vi.waitFor(() => expect(registry.executorInventories[remoteId].status).toBe('failed'));
	...
	it('rejects unqualified stream events and cancels gap recovery when the executor goes offline', async () => {
		const { registry, executors, sent, callbacks, write } = setup();
		await registry.initialize();
		...
		executors.applySnapshot([executor('local'), executor(remoteId, 'offline')]);
	...
	it('retains remote history on offline lists and catches rapid executor loss/ready without browser reconnect', async () => {
		const { registry, executors, sent, callbacks, write, dispose } = setup();
		await registry.initialize();
		const id = terminal(remoteId).terminalId;
		await registry.attach(id, 'restore');
		const old = sent.at(-1)!;
	...
	it('fences a delayed pre-disconnect list across a new authoritative inventory', async () => {
		const { registry, executors, list } = setup();
		await registry.initialize();
		...
		executors.applySnapshot([executor('local'), executor(remoteId, 'offline')]);
		executors.applySnapshot([executor('local'), executor(remoteId)]);
		await vi.waitFor(() => expect(registry.executorInventories[remoteId].status).toBe('ready'));
	...
	it('limits creation per host and updates default labels without replacing renderers', async () => {
		const { registry, executors, list, renderer } = setup();
		...
		executors.applySnapshot([executor('local'), executor(remoteId, 'ready', 'Renamed Host')]);
