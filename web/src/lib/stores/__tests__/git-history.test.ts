...
import { GitHistoryController } from '../git/git-history.svelte';
...
describe('GitHistoryController', () => {
	...
	it('loads the initial commit list', async () => {
		...
		expect(getGitHistoryCommits).toHaveBeenCalledWith('/project', expect.objectContaining({
			limit: 50,
			offset: 0,
		}));
	});
	...
	it('requests visible file bodies and preserves list state on back', async () => {
		...
		history.openCommit('/project', 'abcdef123');
		await flushPromises();
		history.setVisibleRows('/project', [{ kind: 'file-header', filePath: 'a.ts', id: 'row', estimatedHeight: 42 } as never]);
		await flushPromises();
		history.backToList();
