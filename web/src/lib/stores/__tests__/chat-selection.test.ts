...
import { ChatSelectionStore } from '../chat-selection.svelte';

function makeStore(): ChatSelectionStore {
	return new ChatSelectionStore();
}

function selectedIds(store: ChatSelectionStore): string[] {
	return [...store.selectedIds];
}

describe('ChatSelectionStore', () => {
