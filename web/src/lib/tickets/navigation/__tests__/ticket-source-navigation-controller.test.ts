...
import type { TicketSourceResolution } from '$shared/ticket-source-navigation';
import type { TicketBootstrap } from '$shared/tickets';
...
function fixture() {
	...
	let partition: Pick<TicketBootstrap, 'storeId' | 'viewerKey'> | null = {
		storeId: 'store-1',
		viewerKey: 'viewer-1',
	};
	let exists = true;
	...
	const deps = {
		...
		resolve: vi.fn<NonNullable<TicketSourceNavigationDeps['resolve']>>(async () => ({
			kind: 'found',
			target,
		})),
...
it('newer clicks supersede held source lookups, even if abort is ignored', async () => {
	const f = fixture();
	const first = held<TicketSourceResolution>();
...
it.each(['authority', 'partition', 'focus', 'deleted'] as const)(
	...
	async (change) => {
		const f = fixture();
		const read = held<TicketSourceResolution>();
...
it('keeps deletion races and transport failures separate from transcript reloads', async () => {
	...
	await f.open();
	expect(f.deps.notifications.error).toHaveBeenLastCalledWith(
		'Could not open ticket source. Try again.',
	);
	expect(f.deps.notifications.info).not.toHaveBeenCalled();
	expect(f.deps.workspace.showChatInWindow).not.toHaveBeenCalled();
});
