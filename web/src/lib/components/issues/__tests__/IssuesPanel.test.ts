...
describe('Issues surface', () => {
	...
	it('uses the refreshed status when a remote reopen overtakes close confirmation', async () => {
		...
		await fireEvent.click(
			within(await screen.findByRole('dialog')).getByRole('button', { name: 'Close issue' }),
		);
		await waitFor(() => expect(screen.queryByRole('dialog')).toBeNull());
		setItems([{ ...syntheticIssue(), revision: 3 }]);
		release();
		await controller.refresh();
		await tick();
		...
		async (retry) => {
			...
			await fireEvent.click(
				within(await screen.findByRole('dialog')).getByRole('button', {
					name: retry ? 'Retry same request' : 'Close issue',
				}),
			);
			view.unmount();
			release();
	...
	it('restores the close invoker fallback after submitting a closing comment', async () => {
		...
		await fireEvent.click(within(dialog).getByRole('button', { name: 'Close issue' }));
		await waitFor(() => expect(screen.queryByRole('dialog')).toBeNull());
		await controller.refresh();
		await waitFor(() =>
		...
		async (mode) => {
			...
			api.counts.mockRejectedValue(
				new ApiError(
					...
					mode === 'failure' ? 'ISSUE_PROJECT_UNAVAILABLE' : 'ISSUE_COLLECTION_CHANGED',
				),
			);
			await fireEvent.click(
				within(await screen.findByRole('dialog')).getByRole('button', { name: 'Close issue' }),
	...
	it.each(['fields', 'comment-edit'] as const)(
		...
		async (kind) => {
			...
			await fireEvent.click(
				await within(screen.getByRole('region', { name: 'Issue details' })).findByRole('button', {
					name: 'Retry same request',
				}),
			);
			await controller.refresh();
			...
			expect(
				view.container.querySelector(
					kind === 'fields' ? '.issue-title-input' : '[data-comment-id] textarea',
				),
			).toBeNull();
			expect(
				await within(screen.getByRole('region', { name: 'Issue details' })).findByText(
	...
	it('retains text selection through attach followed by an explicit host focus request', async () => {
		...
		render(IssuesTestHost, { controller, frame });
	...
	it('displays the stored priority and submits changed metadata through the shared form', async () => {
		...
		await tick();
		await fireEvent.click(screen.getByRole('button', { name: 'Edit' }));
		const priority = within(screen.getByRole('region', { name: 'Issue details' })).getByLabelText(
			'Priority',
	...
	it('retains close resolution and comment across dialog cancellation and renderer remount', async () => {
		...
		await fireEvent.click(remounted.getByRole('button', { name: 'Close issue' }));
		expect(api.mutate.mock.lastCall?.[0].payload).toMatchObject({
			action: 'close',
			resolution: 'canceled',
	...
	it('creates without a chat or filesystem default and opens authoritative detail', async () => {
		...
		await fireEvent.click(form.getByRole('button', { name: 'Create issue' }));
		await waitFor(() => expect(controller.detail.current?.issue.title).toBe('Human-created work'));
	...
	it('preserves selection and a dirty editor across layout changes and remote refresh', async () => {
		...
		setItems([{ ...syntheticIssue(), title: 'Remote title', revision: 2 }]);
	...
	it('requires a close confirmation and honors cancellation and resolution', async () => {
		...
		await fireEvent.click(form.getByRole('button', { name: 'Close issue' }));
		await waitFor(() => expect(controller.detail.current?.issue.resolution).toBe('canceled'));
	});
