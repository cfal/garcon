...
<div class="border-b border-border last:border-0">
	<!-- File row -->
	<div
		class="group/filerow flex items-center hover:bg-accent/50 {isMobile
			? 'px-2 py-1.5'
			: 'px-3 py-2'}"
	>
		<input
		...
		<div class="flex items-center flex-1">
			...
			>
				<ChevronRight
					class="w-3 h-3 transition-transform duration-200 {isExpanded ? 'rotate-90' : 'rotate-0'}"
				/>
			</button>
			<button
				class="flex-1 truncate text-left {isMobile
					? 'text-xs'
					: 'text-sm'} cursor-pointer hover:text-interactive-accent hover:underline"
				onclick={(e) => {
					e.stopPropagation();
					onFileOpen(filePath);
				}}
			...
			<div class="flex items-center gap-1">
				{#if status === 'M' || status === 'D'}
					<button
						...
						class="{isMobile
							? 'px-2 py-1 text-xs'
							: 'p-1'} hover:bg-status-error rounded text-status-error-foreground font-medium flex items-center gap-1"
				...
				<span
					...
					title={getStatusLabel(status)}>{status}</span
				>
			</div>
		</div>
	</div>
	...
	<div
		class="bg-muted/50 transition-all duration-300 ease-in-out overflow-hidden {isExpanded && diff
