<script lang="ts">
	...
	function modifierButtonClass(mode: ShellModifierMode): string {
		return cn(
			...
			mode === 'inactive' && 'border-border bg-background text-muted-foreground hover:bg-accent hover:text-accent-foreground'
		);
	}

	const toolbarKeyButtonClass = 'inline-flex h-8 min-w-12 flex-none items-center justify-center rounded-md border border-border bg-card px-3 font-mono text-[11px] font-semibold uppercase tracking-[0.16em] text-foreground transition-colors hover:bg-accent hover:text-accent-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring';
</script>
	...
	<div class="h-full flex flex-col bg-terminal-bg w-full">
		...
		{#if showMobileToolbar}
			<div class="flex-shrink-0 border-b border-border bg-background/95 px-2 py-1.5 backdrop-blur supports-[backdrop-filter]:bg-background/75">
				<div class="flex gap-1.5 overflow-x-auto pb-0.5 [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
