...
export function sidebarProjectKey(projectPath: string, executorId?: string | null): string {
	if (effectiveExecutorId(executorId) !== 'local') return `executor:${JSON.stringify([executorId, projectPath])}`;
...
const PROJECT_ACTIVITY_SECTIONS = [
	'inactive',
	'archived',
...
export function sidebarActivitySection(
	...
	if (chat.isPinned) return 'active';
...
function createNestedProjectGroupingContext(chats: ChatSessionRecord[]): ProjectGroupingContext {
	...
	for (const chat of chats) {
		...
		const key = sidebarProjectKey(normalizedPath, executorId);
		...
		const group =
			(project.normalizedPath &&
				projects.find((candidate) =>
					candidate.executorId === project.executorId && isProjectPathAncestor(candidate.normalizedPath, project.normalizedPath),
				)) ||
			project;
...
function projectOrderFromDisplayedChats(
	...
	for (const [index, chat] of chats.entries()) {
		const group = grouping.groupForProjectPath(chat.projectPath, chat.executorId);
...
export function buildSidebarRowModel(input: SidebarRowModelInput): SidebarRowModel {
	...
	const activityGrouped =
		input.grouping === 'project-and-activity' || input.grouping === 'activity';
	const activitySectionByChatId = new Map<string, SidebarChatSection>();
	if (activityGrouped) {
		for (const chat of input.displayedChats) {
			activitySectionByChatId.set(
				chat.id,
				sidebarActivitySection(chat, input.currentTime, input.inactivityDuration),
			);
		}
	}

	if (input.grouping === 'activity') {
	...
	const projectActivityGrouped = input.grouping === 'project-and-activity';
...
function appendSidebarProjectGroups(input: {
	...
	const projectExecutorByKey = new Map<string, string>();
	...
	for (const chat of input.projectChats) {
		...
		projectExecutorByKey.set(key, group.executorId);
	...
	for (const list of input.lists) {
		...
		for (const chatId of input.orders[list]) {
			...
			const project = input.grouping.groupForProjectPath(chat.projectPath, chat.executorId).projectKey;
	...
	for (const project of projectKeys) {
		...
		input.rows.push({
			...
			executorId: projectExecutorByKey.get(project),
...
export function buildSidebarDisplayChatIds(input: {
	...
	const sortedChats =
		input.sortMode === 'recent'
			...
			: input.displayedChats;
	const displayedChats = input.isChatOptimisticallyArchived
		? prioritizeOptimisticArchives(
				sortedChats,
				input.optimisticArchiveOrder ?? input.displayedChats,
				input.isChatOptimisticallyArchived,
			)
		: sortedChats;
	...
	return buildSidebarRowModel({
		...
		inactivityDuration: input.inactivityDuration,
