...
async function mount(empty = false, pinnedProjectPaths: string[] = []) {
	const fixture = ticketTestHarness(empty ? [] : [syntheticTicket()]);
	controllers.push(fixture.controller);
	fixture.controller.setPresentationVisible(true);
	await fixture.controller.refresh();
	const view = render(TicketsTestHost, { controller: fixture.controller, pinnedProjectPaths });
	await tick();
	return { ...fixture, view };
}
function hold() {
...
describe('Tickets stable, immediate interactions', () => {
	...
	it.each([false, true])(
		...
		async (loaded) => {
			...
			await controller.refresh();
			const detail = within(view.container.querySelector<HTMLElement>('.ticket-detail')!);
			expect(detail.getByText('Synthetic refresh unavailable')).toBeTruthy();
			expect(detail.queryByText('Loading ticket…')).toBeNull();
			expect(view.container.querySelector('.ticket-browser')?.textContent).not.toContain(
				'Synthetic refresh unavailable',
			);
			await fireEvent.click(detail.getByRole('button', { name: 'Refresh' }));
			await controller.refresh();
			expect(screen.getByRole('heading', { name: 'Synthetic ticket 1' })).toBeTruthy();
			expect(screen.queryByText('Synthetic refresh unavailable')).toBeNull();
		},
	);

	it.each([1, 2])(
		'keeps a failed mutation visible while another view loads (ticket %s)',
		async (number) => {
			const { controller, api, view } = await mount();
			...
			const ticket = syntheticTicket(number);
			const mutation = controller.mutate(ticket, {
				action: 'update',
				ticketId: ticket.id,
			...
			await tick();
			expect(
				within(view.container.querySelector<HTMLElement>('.ticket-detail-placeholder')!).getByText(
					message,
				),
			).toBeTruthy();
			expect(view.container.querySelector('.ticket-browser > .ticket-notice')).toBeNull();
			readBarrier.release();
			await controller.refresh();
			await tick();
			expect(
				within(screen.getByRole('region', { name: 'Ticket details' })).getByText(message),
			).toBeTruthy();
			await fireEvent.click(screen.getByRole('button', { name: 'Back to tickets' }));
			expect(view.container.querySelector('.ticket-browser > .ticket-notice')?.textContent).toContain(
				message,
			);
			expect(view.container.querySelector('.ticket-detail')).toBeNull();
		},
	);
	...
	it('leaves an empty board plain and removes the redundant heading', async () => {
		...
		expect(view.container.querySelectorAll('.ticket-lane')).toHaveLength(3);
	...
	it.each(['list', 'board'] as const)(
		...
		async (layout) => {
			...
			expect(
				view.container.querySelector('[data-ticket-search-button]')?.getAttribute('aria-busy'),
			).toBe('true');
			barrier.release();
			await controller.refresh();
	...
	it('does not flash outside-filter feedback while changing layouts', async () => {
		...
		controller.createdTicketId = 'G-1';
		const barrier = hold();
		const counts = api.counts.getMockImplementation()!;
		api.counts.mockImplementationOnce(async (...args) => {
			await barrier.promise;
	...
	it.each([false, true])(
		...
		async (changedLane) => {
			...
			api.mutate.mockImplementationOnce(async () => {
				await barrier.promise;
				throw new ApiError(409, 'Synthetic rejected move', 'TICKET_REVISION_CONFLICT');
			});
			...
			expect(
				view.container.querySelector('[data-status="in-review"] [data-ticket-id="G-1"]'),
			).not.toBeNull();
			expect(view.container.querySelector('[data-status="open"] [data-ticket-id="G-1"]')).toBeNull();
			expect(controller.activeLane).toBe('in-review');
			if (changedLane) controller.activeLane = 'in-progress';
			barrier.release();
			await waitFor(() => expect(controller.drafts.pending).toBe(false));
			expect(
				view.container.querySelector('[data-status="open"] [data-ticket-id="G-1"]'),
			).not.toBeNull();
	...
	it.each(['Release', 'All projects'])(
		...
		async (project) => {
			...
			await controller.refresh();
			await fireEvent.click(screen.getByRole('button', { name: 'Search' }));
			const search = screen.getByPlaceholderText('Search tickets…') as HTMLInputElement;
	...
	it.each([false, true])(
		...
		async (assigned) => {
			...
			setItems([
				{ ...syntheticTicket(), assignee: assigned ? { kind: 'user', username: 'local' } : null },
			]);
			await fireEvent.click(screen.getByRole('button', { name: 'Open G-1' }));
			await controller.refresh();
			const detail = screen.getByRole('region', { name: 'Ticket details' });
			const status = within(detail).getByRole('button', { name: 'Change status of G-1' });
			expect(status.classList.contains('ticket-button')).toBe(true);
			expect(
				within(status.parentElement!)
					.getAllByRole('button')
					.map((button) => button.textContent?.trim()),
			).toEqual(['Open', 'Edit', 'Close ticket']);
			...
			expect(detail.querySelector('.ticket-detail-header')?.textContent).not.toContain('G-1');
	...
	it.each(['done', 'canceled'] as const)(
		'offers Reopen only in the status menu for %s tickets',
		async (resolution) => {
			const { controller, setItems, api } = await mount();
			setItems([{ ...syntheticTicket(), status: 'closed', resolution }]);
			controller.select('G-1');
			await controller.refresh();
			const detail = within(await screen.findByRole('region', { name: 'Ticket details' }));
			expect(detail.queryByRole('button', { name: 'Reopen' })).toBeNull();
			expect(detail.queryByRole('button', { name: 'Close ticket' })).toBeNull();
	...
	it.each([false, true])(
		...
		async (composing) => {
			...
			await controller.refresh();
			await fireEvent.click(screen.getByRole('button', { name: 'Search' }));
			const input = screen.getByPlaceholderText('Search tickets…') as HTMLInputElement;
			const label = screen.getByRole('button', { name: 'Label' });
			const clear = screen.getByRole('button', { name: 'Clear' });
			expect(input.closest('.ticket-search-row')?.contains(clear)).toBe(true);
			expect(view.container.querySelector('.ticket-filter-footer')).toBeNull();
			vi.useFakeTimers();
