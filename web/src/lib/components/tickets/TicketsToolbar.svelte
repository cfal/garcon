<script lang="ts">
	import { onDestroy, tick } from 'svelte';
	import Plus from '@lucide/svelte/icons/plus';
	import Search from '@lucide/svelte/icons/search';
	import LoaderCircle from '@lucide/svelte/icons/loader-circle';
	import ChevronUp from '@lucide/svelte/icons/chevron-up';
	import Check from '@lucide/svelte/icons/check';
	import X from '@lucide/svelte/icons/x';
	import type { TicketsController } from '$lib/tickets/catalog/tickets-controller.svelte.js';
	import type { TicketChatSummary } from './ticket-presentation.js';
	import type { TicketListQuery, TicketPriority } from '$shared/tickets';
	import { parseTicketAssigneeQuery } from '$shared/ticket-validation';
	import TicketFacetFilter from './TicketFacetFilter.svelte';
	import TicketViewSettings from './TicketViewSettings.svelte';
	import TicketSearchOptions from './TicketSearchOptions.svelte';
	import * as m from '$lib/paraglide/messages.js';
	let {
		controller,
		chats,
		username,
		onCreate,
		onClose,
		closeDisabled = false,
	}: {
		controller: TicketsController;
		chats: readonly TicketChatSummary[];
		username: string;
		onCreate: () => void;
		onClose?: () => void;
		closeDisabled?: boolean;
	} = $props();
	let filterError = $state(false);
	let searchOpen = $state(false);
	let form = $state<HTMLFormElement | null>(null);
	let searchInput = $state<HTMLInputElement | null>(null);
	let searchText = $derived(controller.query.query ?? '');
	let searchTimer: ReturnType<typeof setTimeout> | undefined;
	const searchId = $props.id();
	const saveFeedback = $derived(controller.saveFeedback);
	const searchBusy = $derived(controller.loading || saveFeedback === 'saving');
	const hasFilters = $derived(
		Object.values(controller.query).some((value) => value !== false && value !== undefined),
	);
	function apply(
		extra: Pick<TicketListQuery, 'project' | 'label' | 'ready' | 'includeClosed'> = {},
	): boolean {
		clearTimeout(searchTimer);
		searchTimer = undefined;
		if (!form) return false;
		const data = new FormData(form);
		const optionalText = (key: string) => String(data.get(key) ?? '').trim() || undefined;
		try {
			const priority = optionalText('priority');
			const assignee = optionalText('assignee');
			controller.setQuery({
				project: controller.query.project,
				query: optionalText('query'),
				status: optionalText('status') as TicketListQuery['status'],
				priority: priority === undefined ? undefined : (Number(priority) as TicketPriority),
				label: controller.query.label,
				assignee: assignee === undefined ? undefined : parseTicketAssigneeQuery(assignee),
				ready: controller.query.ready,
				includeClosed: controller.query.includeClosed,
				...extra,
			});
			filterError = false;
			return true;
		} catch {
			filterError = true;
			return false;
		}
	}
	function scheduleSearch(event: Event) {
		if (event instanceof InputEvent && event.isComposing) return;
		const target = event.target;
		if (!(target instanceof HTMLInputElement) || target.name !== 'query') return;
		clearTimeout(searchTimer);
		searchTimer = setTimeout(() => apply(), 250);
	}
	async function toggleSearch() {
		searchOpen = !searchOpen;
		if (searchOpen) {
			await tick();
			searchInput?.focus();
		} else if (searchTimer !== undefined) apply();
	}
	function clearFilters() {
		clearTimeout(searchTimer);
		searchTimer = undefined;
		const hasInput = form && Array.from(new FormData(form).values()).some((value) => value !== '');
		if (hasFilters || hasInput) controller.setQuery({});
		searchText = '';
		filterError = false;
	}
	onDestroy(() => clearTimeout(searchTimer));
</script>

<header class="tickets-toolbar">
	<form
		bind:this={form}
		class="ticket-filter-form"
		oninput={scheduleSearch}
		onchange={() => apply()}
		onsubmit={(event) => {
			event.preventDefault();
			apply();
		}}
	>
		<div class="ticket-toolbar-heading">
			<TicketFacetFilter {controller} field="project" onSelect={(project) => apply({ project })} />
			<button
				class="ticket-button ticket-icon-button"
				type="button"
				data-ticket-search-button
				aria-busy={searchBusy}
				aria-expanded={searchOpen}
				aria-controls={searchId}
				aria-label={searchOpen ? m.tickets_search_collapse() : m.tickets_filter_apply()}
				title={searchOpen ? m.tickets_search_collapse() : m.tickets_filter_apply()}
				data-active={hasFilters}
				onclick={() => void toggleSearch()}
			>
				{#if searchBusy}<LoaderCircle size={16} class="animate-spin" />
				{:else if saveFeedback === 'saved'}<Check size={16} />
				{:else if searchOpen}<ChevronUp size={16} />
				{:else}<Search size={16} />{/if}
			</button>
			<button
				type="button"
				class="ticket-button ticket-primary ticket-new-button"
				disabled={!controller.bootstrap}
				onclick={onCreate}
				data-ticket-focus={JSON.stringify({ kind: 'toolbar', control: 'new' })}
			>
				<Plus size={15} />{m.tickets_new()}
			</button>
			<TicketViewSettings {controller} />
			{#if onClose}<button
					type="button"
					class="ticket-button ticket-icon-button"
					aria-label={m.tickets_close_view()}
					title={m.tickets_close_view()}
					disabled={closeDisabled}
					onclick={onClose}><X size={16} /></button
				>{/if}
		</div>
		<div id={searchId} class="ticket-search-options" hidden={!searchOpen}>
			<div class="ticket-search-row">
				<label class="ticket-field ticket-search">
					<span class="sr-only">{m.tickets_search()}</span>
					<input
						bind:this={searchInput}
						name="query"
						class="ticket-input"
						placeholder={m.tickets_search()}
						bind:value={searchText}
					/>
				</label>
				<button type="button" class="ticket-button" onclick={clearFilters}
					>{m.tickets_clear_filters()}</button
				>
			</div>
			<TicketSearchOptions {controller} {chats} {username} onFilter={apply} />
		</div>
		<span class="sr-only" role="status">
			{#if saveFeedback === 'saving'}{m.tickets_saving()}
			{:else if saveFeedback === 'saved'}{m.tickets_saved()}
			{:else if controller.loading}{m.tickets_loading()}{/if}
		</span>
	</form>
	{#if filterError}<p class="ticket-notice" role="alert">{m.tickets_filter_error()}</p>{/if}
</header>
