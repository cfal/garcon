<script lang="ts">
	...
	import { getExecutionNodes, getModelCatalog, getLocalSettings, getAppShell } from '$lib/context';
	...
	const nodeId = $derived(handoff.target?.nodeId ?? 'local');
	const catalog = $derived(rootCatalog.forNode(nodeId));
	const basePath = $derived(nodes.get(nodeId)?.projectBasePath ?? '');
	...
	$effect(() => {
		if (!handoff.target || !nodes.isReady(nodeId)) return;
	...
	function handleModelChange(selection: ModelSelectorChange): void {
		if (selection.nodeId === nodeId) handoff.selection = selection;
	}
...
>
	<Dialog.Content class="sm:max-w-lg">
		<Dialog.Header>
			<Dialog.Title>Move to {nodes.label(handoff.target?.nodeId)}</Dialog.Title>
		...
		<form class="space-y-4" onsubmit={handleSubmit}>
			<div class="relative space-y-1">
				...
				<div class="flex gap-2">
					...
					<Button
						...
						disabled={handoff.checking || !nodes.filesAvailable(nodeId)}
						onclick={() => (showBrowser = !showBrowser)}
					>
						<FolderOpen class="size-4" />
					</Button>
				</div>
				{#if showBrowser && nodes.filesAvailable(nodeId)}
					<DirectoryBrowser
						{nodeId}
						nodeContextKey={nodes.pathContextKey(nodeId)}
						{basePath}
			...
			{#if handoff.selection}
				<ComposerModelSelector
					value={{ nodeId, ...handoff.selection }}
			...
			{#if !nodes.isReady(nodeId)}
				<p role="status" class="text-sm text-destructive">Execution node is unavailable</p>
			...
			<Dialog.Footer>
				...
				<Button type="submit" disabled={!handoff.canConfirm}>
					{handoff.checking ? 'Checking...' : 'Use This Node'}
				</Button>
			</Dialog.Footer>
		</form>
	</Dialog.Content>
