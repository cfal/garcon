<script lang="ts">
	...
	<Dialog.Root {open} onOpenChange={handleOpenChange}>
		<Dialog.Content
			class="top-auto bottom-[var(--mobile-nav-total)] left-0 w-full max-w-none translate-x-0 translate-y-0 h-auto max-h-[calc(100dvh-var(--mobile-nav-total))] rounded-t-2xl rounded-b-2xl border-x-0 border-b overflow-x-hidden overflow-y-auto p-0 sm:top-[50%] sm:bottom-auto sm:left-[50%] sm:w-full sm:max-h-[90dvh] sm:max-w-3xl sm:translate-x-[-50%] sm:translate-y-[-50%] sm:rounded-lg sm:border sm:border-x sm:border-b"
