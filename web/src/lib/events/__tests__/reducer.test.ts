...
describe('applyChatMessages', () => {
	it('appends messages to an empty array', () => {
		const result = applyChatMessages([], [new UserMessage('2026-01-01T00:00:00Z', 'Hello')]);
		expect(result).toHaveLength(1);
		expect(result[0].type).toBe('user-message');
	});

	it('appends messages to existing array', () => {
		const current: ChatMessage[] = [new UserMessage('2026-01-01T00:00:00Z', 'Question')];
	...
	it('returns current array unchanged when incoming is empty', () => {
		const current: ChatMessage[] = [new UserMessage('2026-01-01T00:00:00Z', 'Hello')];
		const result = applyChatMessages(current, []);
	...
	it('preserves original array immutably', () => {
		const current: ChatMessage[] = [new UserMessage('2026-01-01T00:00:00Z', 'Hello')];
		const result = applyChatMessages(current, [new AssistantMessage('2026-01-01T00:00:01Z', 'Hi')]);
