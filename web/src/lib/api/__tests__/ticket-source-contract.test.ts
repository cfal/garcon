...
import { resolveTicketSource } from '../ticket-source.js';
...
it('sends a no-store view-qualified source address only on demand', async () => {
	...
	expect(await resolveTicketSource(source, abort.signal)).toEqual({
		kind: 'found',
...
it.each([
	...
	await expect(resolveTicketSource(source, new AbortController().signal)).rejects.toThrow(
		'does not match',
	);
});
...
it('retains typed source failures and propagates target-page cancellation to HTTP', async () => {
	...
	vi.stubGlobal('fetch', fetcher);
	await expect(resolveTicketSource(source, new AbortController().signal)).rejects.toBeInstanceOf(
		ApiError,
	);
	const abort = new AbortController();
	await expect(
