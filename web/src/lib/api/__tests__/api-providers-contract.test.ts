...
it('uses explicit node grants and revisioned global edits with acknowledged shared deletion', async () => {
	...
	expect(fetchMock.mock.calls.map(([url, options]) => [url, options?.method ?? 'GET'])).toEqual([
		...
		['/api/v1/api-provider-assignments?nodeId=local&apiProviderId=synthetic_profile', 'PUT'],
		['/api/v1/api-provider-assignments?nodeId=local&apiProviderId=synthetic_profile', 'DELETE'],
		...
		['/api/v1/api-providers?nodeId=22222222-2222-4222-8222-222222222222', 'POST'],
	]);
