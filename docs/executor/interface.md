...
Status: historical first-stage design, implemented through commit `2bf52dafc` on 2026-09-19 and superseded by subsequent multi-executor and encrypted-transport work. Its single-executor configuration, unencrypted transport, replay, per-session provider lifetime, retry timing, and module inventory are not current operational guidance. Current transport has no replay and keeps native turns alive across disconnects. See [Current Transport](./transport.md), [Files](./files.md), [Git](./git.md), and [Terminals](./terminal.md). The historical second-stage proposal is [Executors In The App](./app-integration.md).
...
The execution boundary is the complete provider-level `AgentIntegration`, not controller-side provider code over remote process primitives. The same provider implementation runs inside `ExecutionRuntime` or in a separate Bun worker behind `RemoteExecutorClient`.
...
Controller: HTTP/WS, chat registry, queues, canonical ledgers
        |
ExecutionRuntimeApi + provider-level AgentIntegration contracts
        |
        +-- ExecutionRuntime -> provider implementations
        |
        +-- RemoteExecutorClient -> authenticated WebSocket/RPC
                                      |
                               Bun worker
                                      |
                               ExecutionRuntime
...
The implemented stage supports one configured executor per controller. Without remote configuration, existing in-process behavior remains the default. With remote configuration, the controller uses that worker's integrations and project filesystem. There is no per-chat executor selector, multi-executor scheduler, or fallback to local execution.
...
| Project inputs | Executor-owned project inspection, optional repository probe, and bounded `@file` expansion. |
...
| Generic process/file/Git/terminal services | Not implemented on `ExecutionRuntimeApi`. Local application services remain available in local mode; remote machine routes explicitly reject. |
...
The executor owns provider startup, binaries, native transport, JSONL/SSE parsing, event translation, native files, discovery, authentication, attachments, compaction, fork, and history import. Provider-specific code stays in `server-agents/<id>/`, behind `@garcon/server-agent-interface`. The worker is not a headless Garcon application: it has no second chat registry, canonical ledger, application queue, or scheduler.

An `AgentIntegration` is shared by that provider's chats on an executor; it is not constructed per chat. Operations, producer bindings, permission responses, and other resources provide the narrower lifetimes. Core provider imports remain confined to [default-agent-integrations.ts](../../server/runtime/agents/default-agent-integrations.ts).

## ExecutionRuntimeApi Contract

The authoritative types live in [executor.ts](../../server-agents/interface/src/contracts/execution-runtime.ts). The current service accessors deliberately do not advertise unimplemented interfaces:

```ts
type ExecutorAvailability = 'ready' | 'reconnecting' | 'offline' | 'disposed';
...
Stable executor identity, transport process `runtimeId`, logical session identity, and executor resource `instanceId` are distinct. A fresh worker serving session constructs a fresh `ExecutionRuntime` and resource scope, even if the worker process survived. Same-session replay preserves references; fresh-session replacement makes old references stale. A reused native session ID does not make an old execution handle valid for a new run.
...
[ProducerBindings](../../server/controller/agents/producer-bindings.ts) installs the exact controller lease route before calling `producers.bind`, and dispatch waits for registration. Events may precede the execution start/resume reply. Every native operation captures its own publisher; an event never discovers the latest transcript sink by chat ID.
...
On the executor, closing a binding cancels admission and best-effort aborts its active native operation, including a session established after closure. A start still pending when its binding closes rejects with `STALE_RESOURCE`; completed operations are never aborted.
...
- [MessageSession](../../server/remote/transport/message-session.ts) owns message ordinals, cumulative receipts, retained frames, duplicate suppression, and terminal continuity failure. It is single-use, not resettable.
- [SessionTransport](../../server/remote/transport/session-transport.ts) buffers inbound replay until the peer replay fence, drains it in order, and then exposes readiness.
- [WebSocketLink](../../server/remote/transport/websocket-link.ts) owns physical dial/listen, authenticated attachment, heartbeats, and creation of replacement logical sessions.
- [ExecutorRpc](../../server/remote/transport/rpc.ts) owns typed application calls, results/errors, cancellation, and producer notifications for one captured `SessionTransport`.
...
Runs must fail before leases close: closing a producer removes its active run and would otherwise suppress the failure event and ownership release. The existing committed terminal path releases coordinator ownership; no new busy predicate or goal-specific owner is added. [server-event-wiring.ts](../../server/controller/server-event-wiring.ts) connects executor availability to [AgentRuntimeRouter.executionSessionLost](../../server/controller/agents/runtime-router.ts).

Replacement requires the same executor ID, integration inventory/static manifests, and configured project base. The previously successful `migrateOwnedStorage()` and `start()` lifecycle calls run against the candidate before promotion. These are lifecycle setup, never execution resume or redelivery. Capability/root changes require controller restart; hot plugin inventory and rolling-version negotiation are out of scope.
...
Either controller or worker restart permits a fresh session without restarting the survivor. [serveExecutionRuntime](../../server/remote/server/executor-rpc-server.ts) removes producer subscriptions, retires RPC, aborts history readers, and attempts bounded cleanup. Fresh service does not wait for proof of native death. Residual native work or missing final rows are accepted best-effort losses, as in local crashes and interruption.
...
A provider-native connection failure is separate from the controller/worker link. For example, an OpenCode SSE failure cannot be repaired by healthy executor WebSocket replay. The provider integration retains responsibility for its native transport/lifecycle semantics.

## Project Inputs, Paths, And Single Queries

### Project Service

Every agent-serving executor exposes the following narrow service, implemented once by [ProjectService](../../server/runtime/projects/project-service.ts) and proxied remotely:
...
`ExecutorPath` is currently a string convention, not a branded type. Declared path fields use `/` separators: `/home/user/project`, `C:/Users/user/project`, and `//server/share/project`. Conversion happens at the executor's native IO boundary. The controller must not apply host `path.resolve()` to worker paths. POSIX literal backslashes, spaces, Unicode, drive roots, and UNC roots retain their meaning; prompt text, argv, environment values, and opaque native-session references are not globally rewritten.

Persisted preamble scopes are checked lexically as portable paths during loading, without querying the controller filesystem. Mutation-time admission checks the executor. Existing chats whose paths do not resolve there remain unavailable; there is no path mapping or automatic migration.

[Remote machine routes](https://github.com/cfal/garcon/blob/2bf52dafc/server/routes/unavailable-machine-services.ts) return `501 OPERATION_UNSUPPORTED` for files, Git, gh, and terminals. Terminal WebSocket requests return typed `terminal-unsupported` errors while chat remains usable. Manual project-path entry remains available. Remote ticket auto-default resolution returns `TICKET_PROJECT_UNAVAILABLE`; explicit projects work in both HTTP and agent-command paths.

### Single Queries And History

`AgentSingleQueryRequest` takes no cwd, project, or temporary-directory intent. Callers provide the required context in the prompt. CLI adapters can use the executor-local [single-query directory helper](../../server-agents/common/src/shared/single-query-control.ts); Direct providers need no directory. Cleanup runs in `finally`. Temporary directories are invocation setup, not security sandboxes or a change to tool-permission policy.
...
[config.ts](https://github.com/cfal/garcon/blob/2bf52dafc/server/execution-nodes/config.ts) currently uses the same loader for controller and worker:

```ts
interface ExecutorWorkerConfig {
  readonly executorId: string;
...
The controller selects remote mode through `GARCON_AGENT_EXECUTOR_CONFIG`; [controller-node.ts](https://github.com/cfal/garcon/blob/2bf52dafc/server/execution-nodes/controller-node.ts) composes the executor and stable integration registry. [worker-main.ts](https://github.com/cfal/garcon/blob/2bf52dafc/server/execution-nodes/worker-main.ts) takes its config path as a positional argument and owns the serving-session lifetime.
...
In controller-dials mode the worker listens; in worker-dials mode the controller listens. Direction changes which endpoint must be reachable, not which side owns the ledger, credentials, or provider code. The execution link has its own `/executor` listener, distinct from the browser-facing Garcon server. Workers emit `executor-listening` and `executor-ready` diagnostics.
...
Local mode remains the default when `GARCON_AGENT_EXECUTOR_CONFIG` is absent. Switching executor configuration is not a native-session migration: provider state and stored paths must already exist on the selected executor. No state transfer, path remapping, or recovery against another machine's files is implied by changing the connection.
...
- Multi-executor routing, hot inventories, worker supervision, durable process attachment, bulk-stream fairness, metrics/soak hardening, and a remote spawned-CLI bridge require separate scope discussion.

## Verification And Source Map

All shipped integrations are wired through [defaultAgentIntegrations](../../server/runtime/agents/default-agent-integrations.ts): Claude, Codex, OpenCode, Pi, Amp, Cursor, Factory, Direct OpenAI-compatible, Direct OpenAI Responses-compatible, and Direct Anthropic-compatible. Interface conformance is not proof of full provider behavioral parity.

| Coverage | Evidence |
| --- | --- |
| All ten integration contracts through local and both remote dial modes | [integration-conformance.test.ts](../../server/remote/__tests__/integration-conformance.test.ts) |
| Bounded send queues, backpressure, connection loss, consumer failure | [message-session.test.ts](../../server/remote/transport/__tests__/message-session.test.ts), [session-transport.test.ts](../../server/remote/transport/__tests__/session-transport.test.ts) |
| Real WebSocket authentication, fresh-session replacement, no mutation retry | [websocket-link.test.js](../../server/remote/transport/__tests__/websocket-link.test.js), [rpc-disconnect.test.ts](../../server/remote/transport/__tests__/rpc-disconnect.test.ts) |
| Async offline rejection and provider query deadlines after setup | [remote-deadlines.test.ts](../../server/remote/__tests__/remote-deadlines.test.ts) |
| Stable facades, stale replies/events, independent restart, bounded cleanup | [session-replacement.test.ts](../../server/remote/transport/__tests__/session-replacement.test.ts), [executor-restart.test.ts](../../integration-tests/tests/server/executor-restart.test.ts) |
| Immutable operation handles, native session-ID reuse, late rows, closed bindings | [execution-lifecycle.test.ts](../../server/remote/__tests__/execution-lifecycle.test.ts), [producer-bindings.test.js](../../server/controller/agents/__tests__/producer-bindings.test.js) |
| Permission single-flight, history cancellation, immutable event snapshots | [resource-lifecycle.test.ts](../../server/remote/__tests__/resource-lifecycle.test.ts) |
| Executor project ownership, portable paths, symlink/size limits, no fallback, disjoint real-process roots | [project-service.test.ts](../../server/runtime/projects/__tests__/project-service.test.ts), [executor-projects.test.ts](../../integration-tests/tests/server/executor-projects.test.ts) |
...
The [CI matrix](../../.github/workflows/integration-tests.yml) runs SACS as `in-process`, `remote-controller-dials`, and `remote-executor-dials`, with the same assertions and serialized fixtures. Controller/worker roots, credentials, and ports are isolated; teardown retains unexpected worker-exit failures. Provider-specific protocol/scripted tests remain with their owning providers rather than being replaced by transport tests.
...
- **Provider code in the controller over generic process RPC:** rejected. Native parsing, files, authentication, transport, and lifecycle belong together with each provider on the executor; OpenCode's loopback HTTP/SSE remains an internal example, not a reason for generic HTTP tunneling.
