...
import type { TicketCliCommand } from '../ticket-args.js';
import { applyTicketStdin } from '../ticket-commands.js';
...
function parse(...args: string[]): TicketCliCommand {
  const command = parseCliArgs(['ticket', ...args], {}, '/working');
  if (command.kind !== 'ticket') throw new Error('Expected ticket command');
  return command;
}

describe('ticket CLI arguments', () => {
  test('exposes ticket without the retired issue alias', () => {
    expect(parse('list').kind).toBe('ticket');
    expect(() => parseCliArgs(['issue', 'list'], {}, '/working')).toThrow('unknown command: issue');
  });
  ...
  test('resolves strict stdin into description, comment and closing comment before submission', () => {
    ...
    for (const args of [['create', '--title', 'Task'], ['comment', 'G-1'],
      ...
      const result = applyTicketStdin(parse(...args, '--stdin'), body);
      expect(result.readsBodyFromStdin).toBe(false);
      expect(JSON.stringify(result.operation)).toContain('Synthetic\\n\\ttext\\u0000');
    }
    expect(() => applyTicketStdin(parse('comment', 'G-1', '--stdin'), ' \n')).toThrow();
    expect(() => applyTicketStdin(parse('comment', 'G-1', '--stdin'), '\ud800')).toThrow();
    expect(() => applyTicketStdin(parse('comment', 'G-1', '--stdin'), 'x'.repeat(49153))).toThrow();
  });
});
