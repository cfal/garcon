...
import type { Ticket, TicketDetail, TicketPage, TicketWriteResult } from '@garcon/common/tickets';
import { main } from '../main.js';
import { createCliOutput } from '../output.js';
import { ticketQueryParams, ticketSearchParams } from '@garcon/common/ticket-query';

const STORE = '22222222-2222-4222-8222-222222222222';
const REQUEST = '11111111-1111-4111-8111-111111111111';
const TS = '2026-01-01T00:00:00.000Z';
const ticket: Ticket = { id: 'G-1', number: 1, revision: 1, title: 'Synthetic task', description: 'Synthetic body',
...
const result: TicketWriteResult = { success: true, storeId: STORE, collectionRevision: 1, ticket };
const page: TicketPage = { storeId: STORE, collectionRevision: 1, items: [], nextBeforeNumber: null };
const detail: TicketDetail = { storeId: STORE, collectionRevision: 1, ticket, links: [],
...
describe('ticket CLI execution', () => {
  test('interrupt diagnostics distinguish read, pre-submission and uncertain mutation phases', async () => {
    for (const args of [['list'], ['read', 'G-1'], ['history', 'G-1'], ['create', '--title', 'Synthetic task', '--project', 'Release']]) {
      ...
      expect(testCase.stderr.join('')).toContain('no ticket mutation was submitted');
    ...
    expect(stdin.stderr.join('')).toContain('no ticket mutation was submitted');
    const interrupt = new AbortController();
    ...
    expect(mutation.stderr.join('')).toContain('the ticket save is not confirmed');
  ...
  test('resolves default and prints frozen retry identity before the first authenticated POST', async () => {
    const testCase = harness((url, body) => {
      ...
      const request = body as HttpTicketMutationRequest;
  ...
  test('encodes assignees without conflating colons, Unicode or unassigned', () => {
    for (const assignee of ['unassigned', { kind: 'user', username: 'é:team' }, { kind: 'chat', chatId: '1000000000000001' }] as const) {
      expect(ticketQueryParams(ticketSearchParams({ assignee }))).toEqual({ assignee });
    }
  });
  ...
  test('keeps terminal errors safe and typed domain failures nonzero', async () => {
    const testCase = harness(() => Response.json({ success: false, error: 'Synthetic \x1b]0;inject\x07\u202e',
      errorCode: 'TICKET_REVISION_CONFLICT', retryable: false }, { status: 409 }));
    expect(await testCase.run(['claim', 'G-1', '--expected-revision', '1', '--request-id', REQUEST, '--expected-store-id', STORE])).toBe(3);
    expect(testCase.stderr.join('')).toContain('TICKET_REVISION_CONFLICT');
