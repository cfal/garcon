...
import { parseTicketBootstrap, parseTicketDetail, parseTicketHistoryPage, parseTicketPage,
  parseTicketProjectDefault } from '@garcon/common/ticket-responses';
import type { TicketHistoryQuery, TicketListQuery, TicketReadQuery } from '@garcon/common/tickets';
...
function parentChatRefsEqual(
  left: ParentChatRef | null,
  right: ParentChatRef | null,
): boolean {
  if (left === null || right === null) return left === right;
  if (left.chatId !== right.chatId || left.relation !== right.relation) return false;
  if (left.relation === 'delegation' || right.relation === 'delegation') return true;
  return left.transcriptViewId === right.transcriptViewId
...
function parseCommandChat(value: unknown, message: string): ChatListResponse['sessions'][number] {
  try {
    return parseChatListResponse({
      sessions: [value],
      total: 1,
      lastSelectedChatId: null,
    }).sessions[0]!;
  } catch (error) {
    throw new CliError('submission', message, 3, { cause: error });
  }
}

function parseForkResponse(value: unknown): ForkChatResponse {
  const raw = record(value);
  if (raw?.success !== true || !Object.hasOwn(raw, 'chat')) {
    throw new CliError('submission', 'server returned an invalid forked chat', 3);
  }
  return {
    success: true,
    chat: parseCommandChat(raw.chat, 'server returned an invalid forked chat'),
  };
}

function parseForkRunResponse(value: unknown): ForkRunCommandResponse {
  const accepted = parseCommandAcceptedResponse(value);
  const raw = record(value);
  if (
    !raw
    || typeof accepted.chatId !== 'string'
    || typeof accepted.turnId !== 'string'
    || !Object.hasOwn(raw, 'chat')
  ) {
    throw new CliError('submission', 'server returned an invalid forked chat', 3);
  }
  const chat = parseCommandChat(raw.chat, 'server returned an invalid forked chat');
  const responseParentChat = Object.hasOwn(raw, 'parentChat')
    ? parseRequiredParentChat(raw)
    : undefined;
  return {
    ...accepted,
    chatId: accepted.chatId,
    turnId: accepted.turnId,
    parentChat: responseParentChat === undefined ? chat.parentChat : responseParentChat,
    chat,
  };
}
...
export class GarconClient {
  ...
  async forkChat(
    ...
    try {
      ...
      if (!signal?.aborted && isAmbiguousSubmissionError(error)) {
        throw new CliError(
          'transport recovery',
          `the fork into chat ${request.chatId} may have reached Garcon; inspect that chat before retrying`,
          3,
          { cause: error },
        );
      }
  ...
  async #submitTurn<TResponse extends AgentTurnCommandResponse>(
    route: string,
    commandType: 'chat-start' | 'agent-run' | 'steer' | 'fork-run',
    request: StartChatCommandRequest | AgentRunCommandRequest | SteerCommandRequest | ForkRunCommandRequest,
  ...
  async getTicketBootstrap(signal?: AbortSignal) {
    return this.#ticketResponse('GET', '/bootstrap', undefined, parseTicketBootstrap, signal);
  }
  ...
  async listTickets(query: TicketListQuery, signal?: AbortSignal) {
    return this.#ticketResponse('GET', `?${ticketSearchParams(query)}`, undefined, parseTicketPage, signal);
  }

  async readTicket(query: TicketReadQuery, signal?: AbortSignal) {
    return this.#ticketResponse('GET', `/detail?${ticketSearchParams(query)}`, undefined, (value) => {
      const detail = parseTicketDetail(value);
      if (detail.ticket.id !== query.ticketId) throw new Error('Ticket ID disagrees with the request');
      return detail;
    }, signal);
  }

  async getTicketHistory(query: TicketHistoryQuery, signal?: AbortSignal) {
    return this.#ticketResponse('GET', `/history?${ticketSearchParams(query)}`, undefined, (value) => {
      const page = parseTicketHistoryPage(value);
      if (page.items.some((item) => item.ticketId !== query.ticketId)) throw new Error('History belongs to another ticket');
      return page;
    }, signal);
  }

  async mutateTicket(request: HttpTicketMutationRequest, signal?: AbortSignal) {
    return this.#ticketResponse('POST', '/mutate', request, (value) => {
      const result = parseTicketWriteResult(value);
  ...
  async #ticketResponse<T>(method: 'GET' | 'POST', route: string, body: unknown,
    parse: (value: unknown) => T, signal?: AbortSignal): Promise<T> {
    const value = await this.#request('tickets', method, `/api/v1/tickets${route}`, body, signal);
