...
- Root `common/` owns cross-layer contracts, codecs, and server-authoritative/provider-neutral semantics. Browser/Bun-compatible policy shared only by web and CLI belongs in `common/client/`; behavior owned by one client stays in that client's domain.
- `common/client/` may depend on platform-compatible root `common/` modules, but root `common/`, `server/`, and `server-agents/` must not import or re-export it. `scripts/__tests__/common-architecture.test.js` enforces this direction.
- Contract parsers stay beside their contracts. Use direct canonical imports and do not add compatibility forwarding paths across ownership layers.
