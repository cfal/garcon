...
describe('issue project defaults', () => {
  let root;
  let options;
  beforeEach(() => {
    root = realpathSync(mkdtempSync(join(homedir(), 'garcon-issue-project-')));
  ...
  const resolve = (directory, signal) => resolveIssueProjectDefault(directory, signal, options);
  ...
  test('honors cancellation before and during a probe', async () => {
    ...
    await expect(resolveIssueProjectDefault(root, during.signal, {
      ...options, git: async () => {
