import type { ExecutionGitService, NodeCallOptions } from '@garcon/server-agent-interface';
...
import type { GitCommandTrace, GitReviewRouteMetrics } from '../git/types.js';
import { GitDomainError } from '../git/git-types.js';
import { gitHttpError } from '../git/git-service.js';
...
export type GitServiceResolver = (nodeId: string) => Promise<ExecutionGitService>;
...
export function createGitRouteService(resolve: GitServiceResolver, timeoutMs = GIT_OPERATION_TIMEOUT_MS): GitRouteService {
  async function invoke<K extends GitMethod>(method: K, options: ExecutionGitRequests[K] & RouteCallOptions): Promise<ExecutionGitResults[K]> {
    const { nodeId, trace, metrics, signal, ...input } = options;
    const target = executionNodeIdFromValue(nodeId);
    validateGitRequest(method, input);
    const service = await resolve(target);
    const call = service[method] as (request: ExecutionGitRequests[K], options: NodeCallOptions) => Promise<ExecutionGitResults[K]>;
    const { diagnostics, ...result } = await call(input, { signal, timeoutMs });
    if (result.nodeId !== target) throw new GitServiceError(isGitMutation(method) ? 'GIT_MUTATION_OUTCOME_UNKNOWN' : 'GIT_INVALID_RESULT', 'Git returned a different execution node');
    if (diagnostics) {
