...
describe('shared transcript routes', () => {
  ...
  // surface could navigate itself here and run same-origin with Garcon.
  it.each(['share-token', 'missing-token'])(
    'denies framing of the shared chat page for %s',
    async (token) => {
      const routes = createRoutes();
      const response = await routes['/shared/:token'].GET(
        new Request(`http://localhost/shared/${token}`),
        new URL(`http://localhost/shared/${token}`),
      );
