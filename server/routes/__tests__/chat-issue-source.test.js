...
const routePath = '/api/v1/chats/issue-source';
...
function fixture() {
  const calls = [];
  let exists = true;
  const routes = wrapRoutes(createChatIssueSourceRoutes({ hasChat: () => exists }, {
    async resolveIssueSource(value, signal) {
...
test('preserves no-store cancellation without treating it as a server failure', async () => {
  ...
  const routes = wrapRoutes(createChatIssueSourceRoutes({ hasChat: () => true }, {
    async resolveIssueSource(_source, signal) {
