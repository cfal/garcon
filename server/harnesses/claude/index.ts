...
import type { Harness, HarnessRuntime } from '../types.js';

interface ClaudeHarnessRuntime extends HarnessRuntime {
...
function createClaudeRuntime(claude: ClaudeProvider): ClaudeHarnessRuntime {
  return {
    async startSession(request: StartSessionRequest): Promise<StartedProviderSession> {
      ...
      claude.startClaudeCliSession(claudeRequest).catch((error: Error) => {
        console.error(`harnesses: claude start failed for chat ${request.chatId}:`, error.message);
      });
...
export function createClaudeHarness(claude: ClaudeProvider): Harness {
  return {
    id: 'claude',
    label: 'Claude',
