import type { ExecutionTerminalService, TerminalAuthority, TerminalPeer, NodeCallOptions } from '@garcon/server-agent-interface';
...
export class TerminalRuntime {
  ...
  service(nodeId: string): LocalExecutionTerminalService {
    ...
    let manager = this.#managers.get(nodeId);
    if (!manager) {
      manager = new TerminalManager({ ...this.options, nodeId, terminalRuntimeId: this.id });
      this.#managers.set(nodeId, manager);
    }
    return new LocalExecutionTerminalService(manager);
  }
...
export class LocalExecutionTerminalService implements ExecutionTerminalService {
  ...
  async create(authority: TerminalAuthority, request: TerminalCreateRequest, options?: NodeCallOptions) {
    this.#check(authority, options);
    if (request.nodeId !== undefined && request.nodeId !== this.manager.nodeId) throw new TerminalError('terminal-validation', 'Terminal node mismatch.');
  ...
  #check(authority: TerminalAuthority, options?: NodeCallOptions): void {
  ...
  #reference(id: string): void {
    ...
    if (!ref || ref.nodeId !== this.manager.nodeId) throw new TerminalError('terminal-validation', 'Invalid terminal target.');
    if (ref.terminalRuntimeId !== this.manager.terminalRuntimeId) throw new TerminalError('terminal-runtime-changed', 'Terminal node restarted.', 409);
  }
}
