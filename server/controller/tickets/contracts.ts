...
import { validateTicketInput } from '../../common/ticket-error.js';
...
export function deriveTicketCaller(principal: ServerPrincipal, fromChatId?: string): TicketCaller {
  if (principal.mode === 'executor') return validateTicketCaller({
    actor: { kind: 'executor', executorId: principal.executorId, declaredChatId: fromChatId ?? null },
    authority: { kind: 'principal', mode: 'executor', key: principal.executorId },
    owner: fromChatId ? { kind: 'chat', chatId: fromChatId } : { kind: 'executor', executorId: principal.executorId },
  });
