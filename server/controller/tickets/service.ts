...
export class TicketService {
  ...
  #requireOwner(owner: TicketOwner, context: TicketMutationContext): void {
    ...
    if (owner.kind === 'executor') {
      if (context.actor.kind !== 'executor' || owner.executorId !== context.actor.executorId) {
        throw new TicketDomainError('TICKET_VALIDATION_FAILED', 'An executor assignment must name the authenticated executor.');
      }
      return;
    }
