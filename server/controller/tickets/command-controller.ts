  ...
  async #mutate(source: AgentCommandSource, command: GarconTicketMutationCommand,
    ...
    const prepared = await this.#locked(source, signal, storeId, () => {
      ...
      if (payload.action === 'create' && payload.input.project === undefined) {
        ...
        return { kind: 'resolve-project' as const, directory: chat.projectPath, executorId: effectiveExecutorId(chat.executorId) };
      }
      return { kind: 'complete' as const, result: this.#execute(source, command, context) };
    });
    if (prepared.kind === 'complete') return prepared.result;
    const probe = await this.options.resolveProject(prepared.directory, signal, prepared.executorId).then(
    ...
    return this.#locked(source, signal, storeId, () => {
      ...
      const chat = this.options.registry.getChat(source.chatId)!;
      if (chat.projectPath !== prepared.directory || effectiveExecutorId(chat.executorId) !== prepared.executorId) throw sourceUnavailable();
