...
function fixture(resolveProject) {
  ...
  const controller = new TicketCommandController({
    ...
    registry: { getChat: (id) => f.chats.has(id) ? { projectPath: current.directory, executorId: current.executorId } : null },
    notices: {
...
describe('ticket command controller', () => {
  test('unavailable executors reject automatic defaults while permitting explicit projects', async () => {
  ...
  test('resolves automatic projects on the captured executor and fences same-path executor changes', async () => {
    const held = Promise.withResolvers();
    const entered = Promise.withResolvers();
    const remote = '11111111-1111-4111-8111-111111111111';
    const f = fixture(async (directory, _signal, executorId) => {
      expect(directory).toBe('/synthetic');
      expect(executorId).toBe(remote);
      entered.resolve();
      return held.promise;
    });
    f.current.executorId = remote;
    const response = f.send(create);
    await entered.promise;
    f.current.executorId = 'local';
