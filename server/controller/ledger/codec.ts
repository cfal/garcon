...
function parseAgentSwitch(value: unknown): LedgerAgentSwitchDetail {
  ...
  for (const key of ['fromExecutorId', 'toExecutorId']) {
    if (detail[key] !== undefined && !isExecutorId(detail[key])) throw new Error('Invalid agent switch executor');
  }
  return {
    ...(typeof detail.fromExecutorId === 'string' ? { fromExecutorId: detail.fromExecutorId } : {}),
    ...(typeof detail.toExecutorId === 'string' ? { toExecutorId: detail.toExecutorId } : {}),
