...
export class SessionCommands {
  ...
  private async updateProjectPathLocked(input: UpdateProjectPathInput): Promise<ProjectPathPatchResponse> {
    ...
    const nextProjectPath = await resolveUpdatedProjectPath(input.projectPath, this.deps.inspectProject, chat.executorId);
  ...
  private async nativeSessionForProjectPathUpdate(
    ...
    if (this.deps.agents.requiresNativePathForProjectPathUpdate(chat.agentId, chat.executorId)) {
      throw new CommandValidationError(
