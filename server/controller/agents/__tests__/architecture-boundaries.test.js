...
describe('agent architecture boundaries', () => {
  test('moves every provider implementation into an isolated package', () => {
    for (const providerId of providerIds) {
      ...
      expect(existsSync(`server/controller/agents/${providerId}`), providerId).toBe(false);
    }
    expect(existsSync('server/acp')).toBe(false);
    expect(existsSync('server/controller/agents/direct')).toBe(false);
  });
    ...
    for (const file of walk('server')) {
      ...
      for (const packageName of providerPackages) {
        if (source.includes(packageName)) {
          expect(relative('.', file), packageName).toBe('server/runtime/agents/default-agent-integrations.ts');
        }
      }
    }
  });
  ...
  test('fences agent actions synchronously and stops handoff recovery before shutdown drains dependencies', () => {
    const source = readFileSync('server/controller/server.ts', 'utf8');
  ...
  test('[TLV5-L12.01-STATIC-01] keeps generic server modules out of provider implementation paths', () => {
    for (const root of ['server/controller/chats', 'server/controller/routes', 'server/controller/ws']) {
  ...
  test('keeps ledger conversation projection out of the universal execution contract', () => {
    const productionSources = [
      ...
      ...walk('server/controller/agents').filter((file) => !file.includes('__tests__')),
      ...walk('server/runtime/agents').filter((file) => !file.includes('__tests__')),
    ];
    for (const file of productionSources) {
  ...
  test('discovers build contributions from package metadata', () => {
    ...
    expect(source).not.toContain('server/controller/agents/pi');
    expect(source).not.toContain('server/controller/chats/search/worker');
  });
});
