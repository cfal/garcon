...
export class AgentHandoffService {
  ...
  async resolveTarget(input: {
    ...
    const executorId = effectiveExecutorId(requested.executorId === undefined ? input.chat.executorId : requested.executorId);
    if (requested.agentId === input.chat.agentId && executorId === effectiveExecutorId(input.chat.executorId)) {
      throw new DomainError(
        'VALIDATION_FAILED',
  ...
  #applyLedgerBoundary(intent: AgentHandoffIntent): void {
    ...
    const marker = markers[0]
      ?? this.deps.ledger.appendAgentSwitch(intent.chatId, intent.watermark.viewId, {
        ...
        fromExecutorId: effectiveExecutorId(intent.source.executorId),
        toExecutorId: effectiveExecutorId(intent.target.execution.executorId),
...
function matchesSwitchMarker(
  ...
  return marker.detail.fromAgentId === intent.source.agentId
    ...
    && effectiveExecutorId(marker.detail.fromExecutorId) === effectiveExecutorId(intent.source.executorId)
    && effectiveExecutorId(marker.detail.toExecutorId) === effectiveExecutorId(intent.target.execution.executorId)
...
function matchesOwnershipFence(
  ...
  return entry?.agentId === expected.agentId
    && effectiveExecutorId(entry.executorId) === expected.executorId
