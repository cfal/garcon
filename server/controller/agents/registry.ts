...
export interface AgentRegistryServiceContract {
  ...
  supportsFork(agentId: string, executorId?: string | null): boolean;
  ...
  supportsFileAttachmentMimeType(agentId: string, mimeType: string, executorId?: string | null): boolean;
  requiresStrictModelDiscovery(agentId: string, executorId?: string | null): boolean;
  ...
  normalizeThinkingModeForAgent(agentId: string, value: unknown, executorId?: string | null): ThinkingMode;
  launchAgentAuthLogin(agentId: string, executorId?: string | null): Promise<AgentAuthLoginLaunchResult>;
  completeAgentAuthLogin(agentId: string, sessionId: string, code: string, executorId?: string | null): Promise<AgentAuthLoginCompleteResult>;
  getAgentAuthLoginStatus(agentId: string, expectedSessionId?: string, executorId?: string | null): Promise<AgentAuthLoginStatus>;
...
export class AgentRegistry implements AgentRegistryServiceContract {
  ...
  constructor(args: {
    ...
    executors?: ExecutionIntegrationDirectory;
    ...
    this.#directory = new AgentDirectory(args.integrations, args.executors);
  ...
  assertExecutionModeSelectionSupported(agentId: string, selection: {
    ...
    const descriptor = this.#directory.get(agentId, selection.executorId)?.descriptor;
  ...
  normalizeThinkingModeForAgent(agentId: string, value: unknown, executorId?: string | null): ThinkingMode {
    return normalizeSupportedThinkingMode(
      value,
      this.#directory.require(agentId, executorId).descriptor.supportedThinkingModes,
    );
  }
  supportsAuthLogin(agentId: string, executorId?: string | null): boolean { return Boolean(this.#directory.get(agentId, executorId)?.auth?.launchLogin); }
  supportsAuthLoginCompletion(agentId: string, executorId?: string | null): boolean { return Boolean(this.#directory.get(agentId, executorId)?.auth?.completeLogin); }
  supportsFork(agentId: string, executorId?: string | null): boolean { return this.#directory.has(agentId, executorId); }
  singleQueryRunsToolsWithoutPermission(agentId: string, executorId?: string | null): boolean {
    return this.#directory.get(agentId, executorId)?.singleQuery?.runsToolsWithoutPermission ?? false;
  }
  ...
  supportsImages(agentId: string, executorId?: string | null): boolean { return this.#directory.get(agentId, executorId)?.descriptor.supportsImages ?? false; }
  supportsFileAttachmentMimeType(agentId: string, mimeType: string, executorId?: string | null): boolean {
    return this.#directory.get(agentId, executorId)?.attachments?.fileMimeTypes.includes(mimeType.toLowerCase()) ?? false;
  }

  requiresStrictModelDiscovery(agentId: string, executorId?: string | null): boolean {
    return this.#catalog.requiresStrictModelDiscovery(agentId, executorId);
  }
  ...
  isAgentSessionRunning(agentId: string, agentSessionId: string | null | undefined, executorId?: string | null): boolean {
    return this.#runtime.isAgentSessionRunning(agentId, agentSessionId, executorId);
  }
  ...
  async resolveNativeSession(session: AgentChatEntry, chatId = ''): Promise<AgentNativeSessionRef | null> {
    ...
    if (!this.#directory.isReady(session.executorId)) return session.nativeSession ?? null;
    const integration = this.#directory.get(session.agentId, session.executorId);
  ...
  async launchAgentAuthLogin(agentId: string, executorId?: string | null): Promise<AgentAuthLoginLaunchResult> {
  ...
  async getAgentAuthLoginStatus(agentId: string, expectedSessionId?: string, executorId?: string | null): Promise<AgentAuthLoginStatus> {
    return this.#directory.require(agentId, executorId).auth?.loginStatus?.(expectedSessionId)
  ...
  async getAgentReadinessMap(authByAgent?: Record<string, unknown>, executorId?: string | null) {
    ...
    return Object.fromEntries(this.#directory.list(executorId).map((integration) => {
      ...
      const endpointReady = integration.endpoints !== null
        && this.#catalog.hasEndpointModels(integration.descriptor.id, executorId);
  ...
  getAgentCatalogEntries(executorId?: string | null) { return this.#catalog.getAgentCatalogEntries(executorId); }
}
