...
describe('testGenerationModel', () => {
  ...
  it.each(['chatTitle', 'agentSwitchCompaction', 'commitMessage', 'promptRefinement'])('tests %s on the explicit executor without discovery or fallback', async (target) => {
    const executorId = '22222222-2222-4222-8222-222222222222';
    const config = { executorId, agentId: 'codex', model: 'synthetic-model', thinkingMode: 'none' };
    ...
    expect(harness.runSingleQuery.mock.calls[0][1]).toMatchObject({ executorId, agentId: 'codex', model: 'synthetic-model' });
    harness.runSingleQuery.mockImplementationOnce(async () => { throw new Error('Executor offline'); });
    await expect(testGenerationModel(input)).rejects.toMatchObject({ code: 'GENERATION_TEST_FAILED' });
    expect(harness.runSingleQuery.mock.calls.map(([, options]) => options.executorId)).toEqual([executorId, executorId]);
    expect(harness.agents.getAgentCatalogEntries).not.toHaveBeenCalled();
    expect(harness.agents.getAgentAuthStatusMap).not.toHaveBeenCalled();
  });
  ...
  it('Auto discovers and invokes only Local', async () => {
    ...
    expect(harness.runSingleQuery.mock.calls[0][1].executorId).toBe('local');
  });
