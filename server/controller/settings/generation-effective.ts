...
import { generationSelectionExecutorError } from '../../../common/settings.js';
...
export function resolveEffectiveGenerationConfig({
  ...
  const executorError = generationSelectionExecutorError(cfg);
  if (executorError) throw new ValidationDomainError(executorError);
  const executorId = parseExecutorId(cfg.executorId)!;
...
export function resolveGenerationUiSnapshot(
  ...
  if (generationSelectionExecutorError(input.persisted)) return undefined;
