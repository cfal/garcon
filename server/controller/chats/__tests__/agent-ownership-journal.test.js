...
function createRegistry(initialEntries) {
  ...
  return {
    ...
    installAgentOwnership: mock(async (chatId, { executorId, projectPath, patch }) => {
      const current = entries.get(chatId);
      if (!current) return null;
      Object.assign(current, patch, { executorId: executorId === 'local' ? undefined : executorId, projectPath });
...
describe('AgentOwnershipJournal', () => {
  ...
  it('reloads and applies executor/path ownership without live integration discovery', async () => {
    ...
    const intent = await journal.decideHandoff(decisionInput(registry, {
      target: { ...target(), executorId, projectPath: '/worker/project' },
    }));
    expect(journal.referencesExecutor(executorId)).toBe(true);
    expect(journal.blocksExecutorRemoval(executorId)).toBe(true);
    ...
    expect(registry.getChat('chat')).toMatchObject({ executorId, projectPath: '/worker/project', agentSessionId: null });
    await restarted.completeHandoff(intent.operationId);
    expect(restarted.referencesExecutor(executorId)).toBe(false);
    expect(restarted.blocksExecutorRemoval(executorId)).toBe(false);
  });
  ...
  it.each(['delete', 'recover-prepared', 'recover-removed'])('retains failed controller-ledger cleanup during executor retirement (%s)', async (mode) => {
    const executorId = '22222222-2222-4222-8222-222222222222';
    const registry = createRegistry({ chat: chat('source-agent', { executorId }) });
    const ledger = { deleteChat: mock(() => {}) };
    ledger.deleteChat.mockImplementationOnce(() => { throw new Error('Synthetic ledger deletion failure'); });
    const release = mock(async () => {});
    let configured = true;
    const options = { workspaceDir, registry, ledger, integrations: createIntegrations(release), isExecutorConfigured: () => configured };
    const journal = new AgentOwnershipJournal(options);
    if (mode !== 'delete') {
      ...
      await writeJournal(workspaceDir, { version: 5, ownershipIntents: [{
        ...
        releaseReferences: [{ ...referenceFor('source-agent'), executorId }],
      }] });
    }
    await journal.initialize();
    ...
    expect(journal.blocksExecutorRemoval(executorId)).toBe(true);
    configured = false;
    await journal.retireRemovedExecutor(executorId);
  ...
  it('retains offline cleanup without blocking executor removal, then retires it when the executor is forgotten', async () => {
    const executorId = '22222222-2222-4222-8222-222222222222';
    const registry = createRegistry({ chat: chat('source-agent', { executorId }) });
    let configured = true;
    const journal = new AgentOwnershipJournal({
      ...
      ledger: { deleteChat: mock(() => {}) },
      isExecutorConfigured: () => configured,
    });
    await journal.initialize();
    ...
    await journal.waitForProviderCleanup();
    expect(journal.referencesExecutor(executorId)).toBe(true);
    expect(journal.blocksExecutorRemoval(executorId)).toBe(false);
    await journal.retireRemovedExecutor(executorId);
  ...
  it('retires removed-executor cleanup on restart without releasing artifacts on another executor', async () => {
    const executorId = '22222222-2222-4222-8222-222222222222';
    const registry = createRegistry({ chat: chat('source-agent', { executorId }) });
    ...
    const restarted = new AgentOwnershipJournal({
      ...options, integrations: createIntegrations(release), isExecutorConfigured: (id) => id === 'local',
    });
    await restarted.initialize();
    await restarted.waitForProviderCleanup();
    expect(release).not.toHaveBeenCalled();
  ...
  it.each([false, true])('retries only the ready executor native cleanup (restart=%s)', async (restart) => {
    const executorId = '22222222-2222-4222-8222-222222222222';
    const otherExecutorId = '33333333-3333-4333-8333-333333333333';
    const registry = createRegistry({
      chat: chat('source-agent', { executorId }),
      other: chat('source-agent', { executorId: otherExecutorId }),
    });
    const release = mock(async () => {});
    ...
    const options = {
      ...
      integrations: { get: (agentId, executor) => available.has(executor) ? native.get(agentId) : null },
    };
    ...
    available.add(executorId);
    await Promise.all([journal.retryProviderCleanup(executorId), journal.retryProviderCleanup(executorId)]);
    expect(release).toHaveBeenCalledTimes(1);
    expect(release.mock.calls[0][0].chat.chatId).toBe('chat');
    expect(journal.referencesExecutor(executorId)).toBe(false);
    expect(journal.referencesExecutor(otherExecutorId)).toBe(true);
  ...
  it('retains a failed readiness cleanup and retries on the next readiness edge', async () => {
    ...
    const journal = new AgentOwnershipJournal({
      workspaceDir, registry: createRegistry({ chat: chat('source-agent', { executorId }) }),
      integrations: createIntegrations(release), ledger: { deleteChat: mock(() => {}) },
    });
    await journal.initialize();
    await journal.delete('chat');
    await journal.waitForProviderCleanup();
    await journal.retryProviderCleanup(executorId);
  ...
  it('retires cleanup queued behind an in-flight release without resurrecting the intent', async () => {
    ...
    const journal = new AgentOwnershipJournal({
      ...
      ledger: { deleteChat: mock(() => {}) }, isExecutorConfigured: () => configured,
    });
    await journal.initialize();
    await journal.delete('chat');
    await started.promise;
    configured = false;
    const retiring = journal.retireRemovedExecutor(executorId);
    pending.reject(new Error('Executor removed'));
    await retiring;
    expect(release).toHaveBeenCalledTimes(1);
    expect(await readJournal(workspaceDir)).toEqual(emptyOwnershipJournalV5());
  });
