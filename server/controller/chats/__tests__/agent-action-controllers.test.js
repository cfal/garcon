...
describe('assistant action controllers', () => {
  ...
  it('inherits current path and explicit bypass permission, uses target defaults and source parentage', async () => {
    ...
    gate.resolve();
    await drain();
    expect(f.agents.getAgentCatalogEntry).toHaveBeenCalledWith('test', { strict: true, executorId: 'local' });
  ...
  it('discovers the inherited remote catalog and carries the executor into child admission', async () => {
    const f = fixture();
    const executorId = '22222222-2222-4222-8222-222222222222';
    f.parent.executorId = executorId;
    f.start.request(SOURCE, START);
    await drain();
    expect(f.agents.getAgentCatalogEntry).toHaveBeenCalledWith('test', { strict: true, executorId });
    expect(f.commands.submitAgentCommandStartLocked.mock.calls[0][0]).toMatchObject({ executorId, parentChatId: SOURCE.chatId });
  });
