...
export interface ApiProviderServiceDeps {
  ...
  discoverModels(executorId: string, request: ApiProviderDiscoveryRequest): Promise<ApiProviderModelDiscoveryResponse>;
}
...
export class ApiProviderService {
  ...
  getCatalog(executorId = 'local'): ApiProviderCatalogEntry[] {
    try {
      return this.deps.access.list(executorId).map(redactApiProviderForCatalog);
    } catch (error) {
  ...
  async test(input: ApiProviderInput, executorId = 'local'): Promise<ApiProviderModelDiscoveryResponse> {
    const flat = flattenApiProviderInput(input);
    this.deps.access.assertExecutor(executorId);
    const stored = !flat.apiKey && input.apiProviderId
      ? this.#storedApiKeyForDiscovery({ ...flat, apiProviderId: input.apiProviderId, endpointId: input.endpointId, revision: input.revision }, executorId)
      : null;
    if (stored?.hasKeyForDifferentOrigin) return { success: false, error: 'Enter the API key for this base URL before testing.' };
    return this.deps.discoverModels(executorId, {
      protocol: flat.protocol, baseUrl: flat.baseUrl, apiKey: flat.apiKey || stored?.apiKey,
      modelDiscovery: flat.modelDiscovery ?? 'none',
    });
  }

  async discoverModels(input: ApiProviderModelDiscoveryRequest, executorId = 'local'): Promise<ApiProviderModelDiscoveryResponse> {
    const flat = flattenApiProviderModelDiscoveryInput(input);
    this.deps.access.assertExecutor(executorId);
    const usesCredentials = flat.modelDiscovery !== 'ollama-tags';
    const storedCredential = flat.apiKey ? null : this.#storedApiKeyForDiscovery(flat, executorId);
  ...
  >, executorId: string): StoredDiscoveryCredentialResult {
    ...
    const { endpoint } = this.deps.access.require(executorId, input.apiProviderId, input.endpointId, input.revision);
