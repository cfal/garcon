...
test('controller captures executor, principal mode and exact attachment for controls and cleanup', async () => {
  ...
  const controller = new TerminalController({ requireExecutor(id) { return { getTerminalService: async () => services.get(id) }; } });
  const principal = LOCAL_SERVER_PRINCIPAL;
  try {
    const inventory = await controller.list(principal, remoteId);
    const { terminal } = await controller.create(principal, { executorId: remoteId, expectedTerminalRuntimeId: inventory.terminalRuntimeId, requestId: 'create', requestedInitialWorkingDirectory: null });
...
test('browser close while resolving a service cannot admit a late attachment', async () => {
  ...
  let admitted = false;
  const controller = new TerminalController({ requireExecutor() { return { getTerminalService: () => service.promise }; } });
  const browser = peer();
