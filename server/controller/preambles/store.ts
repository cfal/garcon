  ...
  const preambles = raw.map((valuePreamble) => {
    ...
    if (
      ...
      && preamble.scope.rules.some((rule, index) => (
        ...
        || !isCanonicalExecutorPath(rule.projectPath)
      ))
...
export class PreambleStore {
  ...
  constructor(workspaceDir: string, private readonly retainExecutorReferences?: RetainExecutorReferences) {
  ...
  async #mutate(expectedRevision: number, change: (draft: PreamblesFile) => void): Promise<void> {
    await this.#lock.runExclusive('preambles', async () => {
      ...
      const release = this.retainExecutorReferences?.(preambleExecutors(draft), preambleExecutors(this.#file));
      try {
        try {
...
function preambleExecutors(file: PreamblesFile): (string | null | undefined)[] {
  return file.preambles.flatMap((preamble) => preamble.scope.type === 'project-paths'
    ? preamble.scope.rules.map((rule) => rule.executorId) : []);
}
