...
export function resolvePreambleSelection(
  ...
  for (const id of selection.orderedPreambleIds) {
    ...
    } else if (!preambleScopeMatches(preamble, canonicalProjectPath, executorId)) {
...
export function projectPreambleSelection(
  ...
  const resolved = resolvePreambleSelection(selection, catalog, canonicalProjectPath, executorId);
  return {
...
export function resolveNewChatPreambleSelection(input: {
  ...
  assertPreambleSelectionComposition(
    input.chatId,
    resolvePreambleSelection(selection, input.catalog, input.canonicalProjectPath, input.executorId).eligible,
  );
  return selection;
}
