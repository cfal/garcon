import { parentExecutorPath } from '../../common/portable-path.js';
...
export function preambleCatalogCompositionViolation(
  ...
  for (const [index, preamble] of preambles.entries()) {
    ...
    for (const rule of preamble.scope.rules) {
      const executorId = effectiveExecutorId(rule.executorId);
      const key = JSON.stringify([executorId, rule.projectPath]);
...
function inheritedNestedMask(
  ...
  while (true) {
    const parentPath = parentExecutorPath(cursor);
    if (parentPath === cursor) break;
    parent = candidates.get(JSON.stringify([candidate.executorId, parentPath]));
    if (parent) break;
    cursor = parentPath;
  }
