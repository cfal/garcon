...
export class ScheduledPromptStore {
  ...
  constructor(
    workspaceDir: string,
    private readonly retainExecutorReferences?: RetainExecutorReferences,
  ...
  async restore(scheduledPrompt: ScheduledPrompt): Promise<void> {
    await this.#mutateInternal((draft) => {
      ...
      return true;
    }, false, scheduledPrompt.target.type === 'new-chat' ? [scheduledPrompt.target.executorId] : [],
  ...
  async #mutateInternal<T>(
    ...
    return this.#lock.runExclusive('scheduled-prompts', async () => {
      ...
      draft.revision += 1;
      await this.#publish(draft, inheritedExecutors, inheritedProviders);
      return structuredClone(result);
    });
  }
  ...
  async #publish(
    ...
    const executors = scheduleExecutors(file);
    const release = this.retainExecutorReferences?.(executors, [...scheduleExecutors(this.#file), ...inheritedExecutors]);
    try {
...
function scheduleExecutors(file: ScheduledPromptsFile): string[] {
  return file.prompts.flatMap((prompt) => prompt.target.type === 'new-chat'
    ? [effectiveExecutorId(prompt.target.executorId)] : []);
}
