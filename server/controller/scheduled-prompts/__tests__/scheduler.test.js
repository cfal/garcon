...
describe('scheduled prompt scheduler', () => {
  ...
  it.each(['EXECUTOR_NOT_FOUND', 'EXECUTOR_IN_USE'])('restores a retained previous executor after registration failure (%s)', async (code) => {
    let writable = true;
    const references = new ExecutorReferenceWrites(() => {
      if (!writable) throw new DomainError(code, 'Synthetic unavailable executor', code === 'EXECUTOR_NOT_FOUND' ? 404 : 409);
    });
    const dir = await tempDir();
    ...
    definition.target.executorId = executorId;
    ...
    try {
      const updating = scheduler.update({ id, expectedRevision: created.revision,
        scheduledPrompt: { ...definition, target: { ...definition.target, executorId: undefined } },
      }).catch((error) => error);
      await entered.promise;
      expect(store.get(id).target.executorId).toBeUndefined();
      expect(() => references.assertNoWrites(executorId)).toThrow('A reference to this executor is being saved');
      release.resolve();
      expect(await updating).toMatchObject({ message: 'Synthetic registration failure' });
      expect(store.get(id).target.executorId).toBe(executorId);
      const reopened = new ScheduledPromptStore(dir, references.retain);
      await reopened.init();
      expect(reopened.get(id).target.executorId).toBe(executorId);
      references.assertNoWrites(executorId);
    } finally {
      release.resolve();
