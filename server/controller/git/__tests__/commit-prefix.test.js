...
describe('computeCommonDirPrefix', () => {
  ...
  it('includes filename for a single file', () => {
    expect(computeCommonDirPrefix(['server/runtime/git/git-service.js'])).toBe('server/runtime/git/git-service.js');
  });

  it('trims extension for a single file when trimExtension is true', () => {
    expect(computeCommonDirPrefix(['server/runtime/git/git-service.js'], true)).toBe(
      'server/runtime/git/git-service',
    );
  });

  it('finds common directory prefix across multiple files', () => {
    expect(
      computeCommonDirPrefix(['server/runtime/git/git-service.js', 'server/runtime/git/git-error-classifier.js']),
    ).toBe('server/runtime/git');
  });
  ...
  it('ignores lock files when computing prefix', () => {
    expect(
      computeCommonDirPrefix(['server/runtime/git/git-service.js', 'package-lock.lock', 'go.sum']),
    ).toBe('server/runtime/git/git-service.js');
  });
  ...
  it('handles root-level files with no directories', () => {
  ...
  it('returns a root-level single file name', () => {
    expect(computeCommonDirPrefix(['package.json'])).toBe('package.json');
  });

  it('strips extension from a root-level single file when requested', () => {
...
describe('applyDirPrefix', () => {
  it('prepends prefix to message', () => {
    expect(applyDirPrefix('add a new feature', 'server/runtime/git')).toBe('server/runtime/git: add a new feature');
  });

  it('prepends prefix to conventional commit as-is', () => {
    expect(applyDirPrefix('feat: add feature', 'server/runtime/git')).toBe('server/runtime/git: feat: add feature');
  });

  it('preserves multiline body', () => {
    const message = 'add feature\n\nDetailed description here.';
    expect(applyDirPrefix(message, 'web')).toBe('web: add feature\n\nDetailed description here.');
  });
