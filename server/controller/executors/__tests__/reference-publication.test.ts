...
import { executorConfigGuards } from '../config-guards.js';
...
async function fixture() {
  const root = await fs.mkdtemp(join(tmpdir(), 'executor-reference-publication-'));
  ...
  const executor = await manager.create({ label: 'Synthetic worker', direction: 'executor-connects' });
  const retainExecutorReferences = manager.retainReferences;
  const chats = new ChatRegistry(root, { retainExecutorReferences, saveDelayMs: 60_000 });
  const settings = new SettingsStore(root, { retainExecutorReferences });
  const preambles = new PreambleStore(root, retainExecutorReferences);
  const schedules = new ScheduledPromptStore(root, retainExecutorReferences);
  await chats.init();
  await settings.init();
  ...
  const ownership = new AgentOwnershipJournal({
    workspaceDir: root, registry: chats, retainExecutorReferences,
    integrations: { get: () => null, require: () => { throw new Error('No provider needed'); } },
    ledger: { deleteChat: () => {} },
  });
  await ownership.initialize();
  manager.setGuards(executorConfigGuards({
    chats, ownership, execution: { ownsExecution: () => false },
  }));
  const publish = (kind: Kind, executorId = executor.id): Promise<unknown> => {
    switch (kind) {
      case 'settings': return settings.setUiSettings({ promptRefinement: {
        executorId, agentId: 'test', model: 'synthetic-model',
      } });
      case 'preamble': return preambles.create({
        ...
        scope: { type: 'project-paths', rules: [{ executorId, projectPath: '/workspace/project', includeNested: false }] },
      ...
      case 'schedule': return schedules.create({
        ...
        target: {
          type: 'new-chat', executorId, agentId: 'test', model: 'synthetic-model', projectPath: root,
      ...
      case 'handoff': return ownership.decideHandoff({
        ...
        target: {
          executorId, agentId: 'test', model: 'synthetic-model', projectPath: root,
          apiProviderId: null, modelEndpointId: null, modelProtocol: null,
  ...
  return { root, manager, executor, chats, settings, preambles, schedules, ownership, publish };
}
...
test.each(kinds)('Delete fences an in-flight %s publication but only pending handoffs block after commit', async (kind) => {
  const { manager, executor, publish } = await fixture();
...
test.each(kinds)('a rejected %s save releases its reference reservation', async (kind) => {
  ...
  await gate.entered;
  await expect(manager.remove(executor.id)).rejects.toMatchObject({ status: 409 });
  gate.release();
...
test.each(kinds)('a %s publication cannot start once Delete owns the executor', async (kind) => {
  const { manager, executor, publish } = await fixture();
  const gate = gateRename('executors.json');
  const deleting = manager.remove(executor.id);
  await gate.entered;
  await expect(publish(kind)).rejects.toMatchObject({ code: 'EXECUTOR_IN_USE', status: 409 });
  gate.release();
  await deleting;
...
test('saved chats do not prevent removal, but active execution does', async () => {
  const { root, manager, executor, chats, ownership } = await fixture();
  const id = await addRemoteChat(chats, root, executor.id);
  let active = true;
  manager.setGuards(executorConfigGuards({ chats, ownership, execution: { ownsExecution: (chat) => active && chat === id } }));
  await expect(manager.remove(executor.id)).rejects.toMatchObject({ code: 'EXECUTOR_IN_USE' });
  active = false;
  await manager.remove(executor.id);
  expect(chats.getChat(id)?.executorId).toBe(executor.id);
  expect(manager.isReady(executor.id)).toBe(false);
  expect(() => manager.requireExecutor(executor.id)).toThrow('Executor is unavailable');
});

test('chat creation rechecks configuration at synchronous registry publication', async () => {
  const { manager, executor, chats, root } = await fixture();
  const gate = gateRename('executors.json');
  const deleting = manager.remove(executor.id);
  await gate.entered;
  const entry = { id: '1767225600000001', executorId: executor.id, agentId: 'test', model: 'synthetic-model', projectPath: root, parentChat: null };
  expect(() => chats.addChat(entry)).toThrow('This executor is being changed');
  expect(chats.getChat(entry.id)).toBeNull();
  chats.addChat({ ...entry, executorId: undefined });
  gate.release();
  await deleting;
  expect(chats.getChat(entry.id)?.executorId).toBeUndefined();
});

async function addRemoteChat(chats: ChatRegistry, root: string, executorId: string, withSettings = false) {
  const id = '1767225600000001';
  chats.addChat({
    id, executorId, agentId: 'test', model: 'synthetic-model', projectPath: root, parentChat: null,
...
test.each(['confirmed', 'failed', 'uncertain'] as const)(
  'compensating chat removal retains its executor until a confirmed removing write (%s)', async (outcome) => {
    const { manager, executor, chats, root } = await fixture();
    const id = await addRemoteChat(chats, root, executor.id);
    ...
    await expect(manager.remove(executor.id)).rejects.toMatchObject({ code: 'EXECUTOR_IN_USE' });
    expect(JSON.parse(await fs.readFile(join(root, 'chats.json'), 'utf8')).sessions[id].executorId).toBe(executor.id);
    const open = fs.open;
...
test('a write captured before chat removal cannot release the removed executor', async () => {
  const { manager, executor, chats, root } = await fixture();
  const id = await addRemoteChat(chats, root, executor.id);
...
test.each([false, true])('deletion inherits an unknown persisted chat executor (provider reference: %s)', async (withSettings) => {
  ...
  const id = await addRemoteChat(persisted, root, executorId, withSettings);
  const chats = new ChatRegistry(root, { retainExecutorReferences: manager.retainReferences, saveDelayMs: 60_000 });
  await chats.init();
  const ownership = new AgentOwnershipJournal({
    workspaceDir: root, registry: chats, retainExecutorReferences: manager.retainReferences,
    integrations: { get: () => null, require: () => { throw new Error('No provider needed'); } },
    ledger: { deleteChat: () => {} },
  });
  await ownership.initialize();
...
test('deletion without a provider reference retains the source during registry removal', async () => {
  const { manager, executor, chats, root, ownership } = await fixture();
  const id = await addRemoteChat(chats, root, executor.id);
  const gate = gateRename('chats.json');
  const deleting = ownership.delete(id);
  await gate.entered;
  expect(chats.getChat(id)).toBeNull();
  expect(ownership.referencesExecutor(executor.id)).toBe(false);
  await expect(manager.remove(executor.id)).rejects.toMatchObject({ code: 'EXECUTOR_IN_USE' });
  gate.release();
  await deleting;
  await ownership.waitForProviderCleanup();
...
test('handoff inherits the persisted source executor but validates the new target', async () => {
  const { root, manager, executor } = await fixture();
  const unknown = '44444444-4444-4444-8444-444444444444';
  const persisted = new ChatRegistry(root);
  await persisted.init();
  const id = await addRemoteChat(persisted, root, unknown);
  const chats = new ChatRegistry(root, { retainExecutorReferences: manager.retainReferences });
  await chats.init();
  const ownership = new AgentOwnershipJournal({
    workspaceDir: root, registry: chats, retainExecutorReferences: manager.retainReferences,
    integrations: { get: () => null, require: () => { throw new Error('No provider needed'); } },
  ...
  const request = {
    ...
    watermark: { viewId: 'synthetic-view', ordinal: 0 },
    target: {
      executorId: '55555555-5555-4555-8555-555555555555', agentId: 'test', model: 'synthetic-model', projectPath: root,
      apiProviderId: null, modelEndpointId: null, modelProtocol: null,
      permissionMode: 'default' as const, thinkingMode: 'none' as const,
      agentSettings: { ownerId: 'test', schemaVersion: 1, values: {} },
    },
  };
  await expect(ownership.decideHandoff(request)).rejects.toMatchObject({ code: 'EXECUTOR_NOT_FOUND' });
  await ownership.decideHandoff({ ...request, target: { ...request.target, executorId: executor.id } });
  expect(ownership.referencesExecutor(unknown)).toBe(true);
  expect(ownership.referencesExecutor(executor.id)).toBe(true);
});

test('reference reservations are executor-scoped, counted, and acquired all-or-nothing', () => {
  const guard = new ExecutorReferenceWrites((id) => { if (id === 'missing') throw new Error('Missing executor'); });
  const releaseFirst = guard.retain(['first', 'first', 'local']);
  const releaseSecond = guard.retain(['first']);
  expect(() => guard.retain(['second', 'missing'])).toThrow('Missing executor');
  guard.assertNoWrites('second');
  guard.assertNoWrites('local');
  releaseFirst();
  releaseFirst();
  expect(() => guard.assertNoWrites('first')).toThrow('A reference to this executor is being saved');
  releaseSecond();
