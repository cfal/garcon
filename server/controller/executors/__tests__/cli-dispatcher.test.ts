...
test('raw route parity preserves query multiplicity, JSON errors, executor authority and restart fences', async () => {
  let calls = 0;
  const pair = cliPair(dispatcher({ '/api/v1/chats': { GET: (req, url, server, context) => {
    ...
    expect(context?.principal).toEqual({ mode: 'executor', key: CLI_EXECUTOR_ID, executorId: CLI_EXECUTOR_ID, expiresAtMs: null });
