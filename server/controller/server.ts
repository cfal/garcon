...
import { executorConfigGuards } from './executors/config-guards.js';
...
import { acquireControllerLease } from './lib/workspace-lease.js';
import {
  advertisedServerUrl,
...
export async function startServer(): Promise<void> {
  ...
  try {
    ...
    if (process.env.GARCON_AGENT_EXECUTOR_CONFIG) {
      throw new Error('GARCON_AGENT_EXECUTOR_CONFIG is no longer supported. Add executors in the app and use a fresh workspace for experimental remote-only chats.');
    }
    ...
    const executors = await ExecutorManager.create({
      ...
      async resolveCredential({ executorId, reference, signal }) {
        ...
        return providerAccess.resolveCredential(executorId, reference);
      },
    });
    const retainExecutorReferences = executors.retainReferences;
    const retainProviderReferences = apiProviderStore.referenceWrites.retain;
    const providerPolicy = createApiProviderPolicy(apiProviderStore, executors, workspaceDir);
    const { access, assignments: providerAssignments } = providerPolicy;
    const terminalManager = new TerminalController(executors);
    const terminalStream = new TerminalStreamHandler(terminalManager);
    const chatRegistry = new ChatRegistry(workspaceDir, { retainExecutorReferences, retainProviderReferences });
    const settings = new SettingsStore(workspaceDir, { retainExecutorReferences, retainProviderReferences });
    ...
    const integrationRegistry = executors.localIntegrations;
    const directory = new AgentDirectory(integrationRegistry, executors);
    const { inspectProject, resolveFileMentions } = executors;
    const projectBasePath = executors.localInfo.projectBasePath;
    const resolveTicketProject = createTicketProjectResolver((executorId) => executors.projectService(executorId));
    const endpointResolver = new ApiProviderEndpointResolver(
      (executorId) => access.list(executorId),
      (agentId, executorId) => directory.get(agentId, executorId)?.descriptor.supportedEndpointProtocols ?? [],
    ...
    const agentOwnership = new AgentOwnershipJournal({
      workspaceDir,
      retainExecutorReferences,
      retainProviderReferences,
      registry: chatRegistry,
      integrations: directory,
      ledger: transcriptLedger,
      isExecutorConfigured: (executorId) => executorId === 'local' || executors.config.get(executorId) !== null,
    });
    ...
    executors.onAvailabilityChanged((executorId) => modelCatalogResponseCache.clear(executorId));
    executors.onChanged(() => {
      for (const executor of executors.list()) {
        if (!executor.enabled) modelCatalogResponseCache.clear(executor.id);
      }
    });
    ...
    const chatListProjector = new ChatListProjector({
      ...
      canReloadFromNativeHistory(_chatId, session) {
        return Boolean(
          ...
          && directory.get(session.agentId, session.executorId)?.nativeHistoryImport,
        );
      },
    });
    ...
    const scheduledPromptStore = new ScheduledPromptStore(workspaceDir, retainExecutorReferences, retainProviderReferences);
    ...
    executors.setGuards(executorConfigGuards({
      chats: chatRegistry, execution: queue, ownership: agentOwnership,
    }));
    let configuredExecutors = new Set(executors.config.list().map((executor) => executor.id));
    executors.onChanged(() => {
      const current = new Set(executors.config.list().map((executor) => executor.id));
      for (const id of configuredExecutors) {
        if (!current.has(id)) {
          void settings.forgetExecutor(id).catch((error) => logger.warn('Could not prune executor preferences', error));
          void providerAssignments.prune([...current]).catch(() => logger.warn('Could not prune provider assignments'));
          void agentOwnership.retireRemovedExecutor(id).catch((error) => logger.warn('Could not retire removed-executor cleanup', error));
        }
      }
      configuredExecutors = current;
    });
