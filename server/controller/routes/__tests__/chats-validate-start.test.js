...
import { ProjectService } from '../../../runtime/projects/project-service.ts';
...
const routeDeps = {
  projects: async () => new ProjectService(os.homedir(), () => {}),
  registry,
  settings,
  queue,
...
describe('GET /api/v1/chats/validate-start', () => {
  it('preserves invalid and offline executor HTTP errors', async () => {
    ...
    const failed = createChatRoutes({ ...routeDeps, projects: async () => {
      throw new DomainError('UNAVAILABLE', 'Executor offline', 503);
  ...
  it('reports an executor failure as unavailable, not a missing project', async () => {
    const failed = createChatRoutes({
      ...routeDeps,
      projects: async () => ({ inspect: async () => { throw new AgentCallError('not-dispatched', 'Executor offline'); } }),
    })['/api/v1/chats/validate-start'].GET;
