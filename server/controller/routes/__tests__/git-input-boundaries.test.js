...
import { gitServiceError } from '../../../runtime/git/service-errors.js';
...
async function fixture() {
  ...
  const runtime = new GitRuntime({ executorId: 'local', instanceId: 'regression', projectBasePath: root, assertAvailable() {} });
...
test('a missing child of an escaping symlink never bypasses the executor boundary', async () => {
  ...
  const restricted = new GitRuntime({ executorId: 'local', instanceId: 'restricted', projectBasePath: project, assertAvailable() {} });
...
test.each(['getQuickSummary', 'getWorkingTreeFingerprint', 'getWorkbenchSnapshot'])('a missing project cannot become an unchecked repository during %s', async (method) => {
  ...
  await fs.mkdir(base);
  const runtime = new GitRuntime({ executorId: 'local', instanceId: 'appearing-project', projectBasePath: base, assertAvailable() {} });
  cleanups.push(() => runtime.dispose());
  ...
  })).toMatchObject({ status: 'not-git-repository', executorId: 'local' });
...
test('an invalid base beneath a regular file fails instead of exhausting admission', async () => {
  ...
  const runtime = new GitRuntime({ executorId: 'local', instanceId: 'invalid-base', projectBasePath, assertAvailable() {} });
  cleanups.push(() => runtime.dispose());
