...
mock.module('../../../common/http-body.js', () => ({
  parseJsonBody,
  MalformedJsonError,
}));
...
describe('agent auth login routes', () => {
  ...
  it('creates API providers through the API provider route', async () => {
    ...
    const url = new URL('http://localhost/api/v1/api-providers?executorId=local');
  ...
  it.each([['test', 'test'], ['models', 'discoverModels']])('qualifies %s by executor and preserves typed failures', async (path, method) => {
    const executorId = '22222222-2222-4222-8222-222222222222';
    const url = new URL(`http://localhost/api/v1/api-providers/${path}?executorId=${executorId}`);
    ...
    expect(apiProviders[method]).toHaveBeenCalledWith({ synthetic: true }, executorId);

    apiProviders[method].mockRejectedValueOnce(new DomainError('EXECUTOR_UNAVAILABLE', 'Executor unavailable', 503));
    const unavailable = await handler(request(), url);
    expect(unavailable.status).toBe(503);
    expect(await unavailable.json()).toMatchObject({ errorCode: 'EXECUTOR_UNAVAILABLE' });
    apiProviders[method].mockRejectedValueOnce(new AgentCallError('unknown', 'Executor disconnected'));
    expect((await handler(request(), url)).status).toBe(503);

    apiProviders[method].mockClear();
    url.searchParams.set('executorId', 'invalid');
