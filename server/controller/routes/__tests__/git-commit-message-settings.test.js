...
mock.module('../../../common/http-body.js', () => ({ parseJsonBody, MalformedJsonError }));
...
describe('POST /api/v1/git/generate-commit-message persisted settings', () => {
  ...
  it('selects repository and explicit model executors independently', async () => {
    const repositoryNode = '11111111-1111-4111-8111-111111111111';
    const generationExecutor = '22222222-2222-4222-8222-222222222222';
    parseJsonBody.mockResolvedValue({ executorId: repositoryNode, project: '/executor-only/project', files: ['file'], generationExecutorId: generationExecutor, agentId: 'codex', model: 'synthetic' });
    const response = await handler(makeRequest({}));
    expect(response.status).toBe(200);
    expect(resolveGit).toHaveBeenCalledWith(repositoryNode);
    expect(generateCommitMessageForFiles).toHaveBeenCalledWith(expect.objectContaining({ executorId: generationExecutor, projectPath: '/executor-only/project' }));
  });

  it('reports an unavailable explicit generation executor before reading the repository', async () => {
    const generationExecutor = '22222222-2222-4222-8222-222222222222';
    parseJsonBody.mockResolvedValue({ project: '/project', files: ['file'], generationExecutorId: generationExecutor, agentId: 'codex', model: 'synthetic' });
    ...
    expect(agents.assertAgentAvailable).toHaveBeenCalledWith('codex', generationExecutor);
  ...
  it('keeps Auto generation Local for a remote repository', async () => {
    const repositoryNode = '11111111-1111-4111-8111-111111111111';
    parseJsonBody.mockResolvedValue({ executorId: repositoryNode, project: '/executor-only/project', files: ['file'] });
    const response = await handler(makeRequest({}));
    expect(response.status).toBe(200);
    expect(resolveGit).toHaveBeenCalledWith(repositoryNode);
    expect(generateCommitMessageForFiles).toHaveBeenCalledWith(expect.objectContaining({ executorId: 'local' }));
  });
