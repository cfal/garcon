...
import { GIT_DIFF_LIMITS, GIT_REF_RESULT_LIMITS, GIT_REVIEW_DOCUMENT_LIMITS } from '../../../runtime/git/types.js';
...
const runtime = new GitRuntime({ executorId: 'local', instanceId: 'test-instance', projectBasePath, assertAvailable() {} });
  ...
  it('returns structured snapshot and file bodies for a commit', async () => {
    ...
    try {
      ...
      parseJsonBody.mockImplementation(() =>
        Promise.resolve({
          project: projectPath,
          document: { executorId: snapshot.executorId, instanceId: snapshot.instanceId, documentId: snapshot.documentId },
          files: ['a.txt'],
          purpose: 'visible',
        }),
      );
