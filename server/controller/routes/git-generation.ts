...
import { gitRouteFailure, type GitServiceResolver } from './git-executor-service.js';
import { assertGitBodyUrl, validateGitHttpFields } from './git-request-fields.js';

const GENERATION_FIELDS = ['generationExecutorId', 'agentId', 'model', 'apiProviderId', 'modelEndpointId', 'modelProtocol', 'thinkingMode', 'customPrompt'];
...
function hasGenerationRoutingOverride(input: Record<string, unknown>): boolean {
  return ['generationExecutorId', 'agentId', 'model', 'apiProviderId', 'modelEndpointId', 'modelProtocol'].some((key) => hasOwn(input, key));
}
...
export function createGitGenerationRoute(agents: AgentRegistryServiceContract, settings: SettingsStore, resolveGit: GitServiceResolver) {
  return withJsonBody(async (body: JsonBody, request: Request): Promise<Response> => {
    try {
      ...
      const executorId = hasGenerationRoutingOverride(input) ? executorIdFromValue(input.generationExecutorId) : persistedConfig.executorId;
      ...
      if (agentId) {
        try {
          agents.assertAgentAvailable(agentId, executorId);
          if (hasOwn(input, 'thinkingMode')) {
            agents.assertExecutionModeSelectionSupported(agentId, { thinkingMode: selectedThinkingMode, executorId });
          }
          thinkingMode = agents.normalizeThinkingModeForAgent(agentId, selectedThinkingMode, executorId);
        } catch (error) {
          return jsonErrorFromUnknown(error);
        }
      }

      const repository = await resolveGit(executorIdFromValue(input.executorId));
