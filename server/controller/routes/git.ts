...
export default function createGitRoutes(agents: AgentRegistryServiceContract, settings: SettingsStore, resolveGit: GitServiceResolver, timeoutMs = GIT_OPERATION_TIMEOUT_MS): RouteMap {
  ...
  async function getStatus(request: Request, url: URL): Promise<Response> {
    ...
    return gitJson(git, () => git.getStatus({ executorId: url.searchParams.get('executorId'), signal: request.signal, projectPath: project }));
  }
  ...
  async function postInitialCommit(body: JsonBody, request: Request): Promise<Response> {
    ...
    if (project instanceof Response) return project;
    return gitJson(git, () => git.initialCommit({ executorId: asJsonBody(body).executorId, signal: request.signal, projectPath: project }));
  }
  ...
  async function postCommit(body: JsonBody, request: Request): Promise<Response> {
    return gitJson(git, async () => {
      ...
      return git.commit({ executorId: asJsonBody(body).executorId, signal: request.signal, projectPath: project, message, files });
    });
  }
  ...
  async function getBranches(request: Request, url: URL): Promise<Response> {
    const project = requiredProjectFromQuery(url);
    if (project instanceof Response) return project;
    return gitJson(git, () => git.getBranches({ executorId: url.searchParams.get('executorId'), signal: request.signal, projectPath: project }));
  }
  ...
  async function getRefs(request: Request, url: URL): Promise<Response> {
    ...
    return gitJson(git, () =>
      git.getRefs({ executorId: url.searchParams.get('executorId'),
        projectPath: project,
  ...
  async function postCheckout(body: JsonBody, request: Request): Promise<Response> {
    return gitJson(git, async () => {
      ...
      return git.checkout({ executorId: asJsonBody(body).executorId, signal: request.signal, projectPath: project, ref, refKind });
    });
  }
  ...
  async function postCreateBranch(body: JsonBody, request: Request): Promise<Response> {
    return gitJson(git, async () => {
      ...
      return git.createBranch({ executorId: asJsonBody(body).executorId, signal: request.signal, projectPath: project, branch, baseRef });
    });
  }
  ...
  async function postHistoryCommits(body: JsonBody, request: Request): Promise<Response> {
    return gitJson(git, async () => {
      ...
      const result = await git.getHistoryCommits({ executorId: asJsonBody(body).executorId,
        projectPath: project,
        ref,
        limit,
        offset,
  ...
  async function postCommitSnapshot(body: JsonBody, request: Request): Promise<Response> {
    return gitJson(git, async () => {
      ...
      const result = await measureGitRoutePhase(metrics.phases, 'summary-git', () =>
        git.getCommitSnapshot({ executorId: asJsonBody(body).executorId,
          projectPath: project,
          commit,
          parent,
          context,
  ...
  async function getRemoteStatus(request: Request, url: URL): Promise<Response> {
    const project = requiredProjectFromQuery(url);
    if (project instanceof Response) return project;
    return gitJson(git, () => git.getRemoteStatus({ executorId: url.searchParams.get('executorId'), signal: request.signal, projectPath: project }));
  }

  async function postFetch(body: JsonBody, request: Request): Promise<Response> {
    const project = requiredProjectFromBody(asJsonBody(body));
    if (project instanceof Response) return project;
    return gitJson(git, () => git.fetch({ executorId: asJsonBody(body).executorId, signal: request.signal, projectPath: project }));
  }

  async function postPull(body: JsonBody, request: Request): Promise<Response> {
    const project = requiredProjectFromBody(asJsonBody(body));
    if (project instanceof Response) return project;
    return gitJson(git, () => git.pull({ executorId: asJsonBody(body).executorId, signal: request.signal, projectPath: project }));
  }
  ...
  async function postPush(body: JsonBody, request: Request): Promise<Response> {
    return gitJson(git, async () => {
      ...
      return git.push({ executorId: asJsonBody(body).executorId, signal: request.signal, projectPath: project, remote, remoteBranch });
    });
  }
  ...
  async function getRemotes(request: Request, url: URL): Promise<Response> {
    const project = requiredProjectFromQuery(url);
    if (project instanceof Response) return project;
    return gitJson(git, () => git.getRemotes({ executorId: url.searchParams.get('executorId'), signal: request.signal, projectPath: project }));
  }
  ...
  async function postDiscard(body: JsonBody, request: Request): Promise<Response> {
    return gitJson(git, async () => {
      ...
      return git.discard({ executorId: asJsonBody(body).executorId, signal: request.signal, projectPath: project, file });
    });
  }
  ...
  async function postDeleteUntracked(body: JsonBody, request: Request): Promise<Response> {
    return gitJson(git, async () => {
      ...
      return git.deleteUntracked({ executorId: asJsonBody(body).executorId, signal: request.signal, projectPath: project, file });
    });
  }
  ...
  async function postWorkbenchSnapshot(body: JsonBody, request: Request): Promise<Response> {
    return gitJson(git, async () => {
      ...
      const startedAt = performance.now();
      const result = await measureGitRoutePhase(metrics.phases, 'summary-git', () =>
        git.getWorkbenchSnapshot({ executorId: asJsonBody(body).executorId,
          projectPath: project,
          mode,
          context,
          selectedFile,
  ...
  async function postWorkingTreeFingerprint(body: JsonBody, request: Request): Promise<Response> {
    ...
    return gitJson(git, async () => {
      const trace: GitCommandTrace[] = [];
      const startedAt = performance.now();
      const result = await git.getWorkingTreeFingerprint({ executorId: asJsonBody(body).executorId,
        projectPath: project,
        trace,
        signal: request.signal,
      });
  ...
  async function postQuickSummary(body: JsonBody, request: Request): Promise<Response> {
    ...
    return gitJson(git, async () => {
      const trace: GitCommandTrace[] = [];
      const startedAt = performance.now();
      const result = await git.getQuickSummary({ executorId: asJsonBody(body).executorId,
        projectPath: project,
        trace,
        signal: request.signal,
      });
  ...
  async function postReviewDocumentFiles(body: JsonBody, request: Request): Promise<Response> {
    return gitJson(git, async () => {
      ...
      const metrics: GitReviewRouteMetrics = { phases: [] };
      const startedAt = performance.now();
      const result = await git.getReviewDocumentFileBodies({ executorId: asJsonBody(body).executorId,
        projectPath: project,
        document,
        files,
        purpose,
  ...
  async function postStageSelection(body: JsonBody, request: Request): Promise<Response> {
    return gitJson(git, async () => {
      ...
      const result = await git.stageSelection({ executorId: asJsonBody(body).executorId, signal: request.signal,
  ...
  async function postStageHunk(body: JsonBody, request: Request): Promise<Response> {
    return gitJson(git, async () => {
      ...
      const result = await git.stageHunk({ executorId: asJsonBody(body).executorId, signal: request.signal,
        document: input.document, bodyFingerprint: input.bodyFingerprint, patchDigest: input.patchDigest,
        projectPath: project,
        file,
        mode,
  ...
  async function getWorktrees(request: Request, url: URL): Promise<Response> {
    const project = requiredProjectFromQuery(url);
    if (project instanceof Response) return project;
    return gitJson(git, () => git.getWorktrees({ executorId: url.searchParams.get('executorId'), signal: request.signal, projectPath: project }));
  }

  async function getTargets(request: Request, url: URL): Promise<Response> {
    const project = requiredProjectFromQuery(url);
    if (project instanceof Response) return project;
    return gitJson(git, () => git.getTargetCandidates({ executorId: url.searchParams.get('executorId'), signal: request.signal, projectPath: project }));
  }
  ...
  async function postCreateWorktree(body: JsonBody, request: Request): Promise<Response> {
    return gitJson(git, async () => {
      ...
      return git.createWorktree({ executorId: asJsonBody(body).executorId, signal: request.signal,
        projectPath: project,
        baseRef,
        worktreePath,
        branch,
  ...
  async function postRemoveWorktree(body: JsonBody, request: Request): Promise<Response> {
    return gitJson(git, async () => {
      ...
      return git.removeWorktree({ executorId: asJsonBody(body).executorId, signal: request.signal, projectPath: project, worktreePath, force });
    });
  }
  ...
  async function postCommitIndex(body: JsonBody, request: Request): Promise<Response> {
    return gitJson(git, async () => {
      ...
      return git.commitIndex({ executorId: asJsonBody(body).executorId, signal: request.signal, projectPath: project, message });
    });
  }
  ...
  async function postStagePaths(body: JsonBody, request: Request): Promise<Response> {
    return gitJson(git, async () => {
      ...
      return git.stagePaths({ executorId: asJsonBody(body).executorId, signal: request.signal, projectPath: project, paths, mode });
    });
  }
  ...
  async function postRevertCommit(body: JsonBody, request: Request): Promise<Response> {
    return gitJson(git, async () => {
      ...
      return git.revertCommit({ executorId: asJsonBody(body).executorId, signal: request.signal, projectPath: project, commit });
    });
  }
  ...
  async function getConflicts(request: Request, url: URL): Promise<Response> {
    const project = requiredProjectFromQuery(url);
    if (project instanceof Response) return project;
    return gitJson(git, () => git.getConflicts({ executorId: url.searchParams.get('executorId'), projectPath: project, signal: request.signal }));
  }
  ...
  async function getConflictDetails(request: Request, url: URL): Promise<Response> {
    ...
    return gitJson(git, () =>
      git.getConflictDetails({ executorId: url.searchParams.get('executorId'),
  ...
  async function postAcceptConflictSide(body: JsonBody, request: Request): Promise<Response> {
    return gitJson(git, async () => {
      ...
      const result = await git.acceptConflictSide({ executorId: asJsonBody(body).executorId,
        projectPath: project,
        file,
        side,
        signal: request.signal,
  ...
  async function postMarkConflictResolved(body: JsonBody, request: Request): Promise<Response> {
    return gitJson(git, async () => {
      ...
      return git.markConflictResolved({ executorId: asJsonBody(body).executorId,
        projectPath: project,
        file,
        signal: request.signal,
      });
  ...
  async function getStashes(request: Request, url: URL): Promise<Response> {
    const project = requiredProjectFromQuery(url);
    if (project instanceof Response) return project;
    return gitJson(git, () => git.getStashes({ executorId: url.searchParams.get('executorId'), projectPath: project, signal: request.signal }));
  }
  ...
  async function postCreateStash(body: JsonBody, request: Request): Promise<Response> {
    ...
    if (project instanceof Response) return project;
    return gitJson(git, () =>
      git.createStash({ executorId: asJsonBody(body).executorId,
        projectPath: project,
  ...
  async function getFileHistory(request: Request, url: URL): Promise<Response> {
    ...
    return gitJson(git, () =>
      git.getFileHistory({ executorId: url.searchParams.get('executorId'),
        projectPath: input.project,
        file: input.file,
        limit,
        signal: request.signal,
  ...
  async function getBlame(request: Request, url: URL): Promise<Response> {
    ...
    return gitJson(git, () =>
      git.getBlame({ executorId: url.searchParams.get('executorId'),
        projectPath: input.project,
        file: input.file,
        ref,
        limit,
  ...
  async function getGraph(request: Request, url: URL): Promise<Response> {
    ...
    return gitJson(git, () => git.getGraph({ executorId: url.searchParams.get('executorId'), projectPath: project, limit, signal: request.signal }));
  }
