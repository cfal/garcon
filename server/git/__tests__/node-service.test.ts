...
async function fixture(registry?: GitReviewDocumentRegistry) {
  ...
  const root = await fs.mkdtemp(path.join(temporary, 'git-node-'));
  ...
  const configuration = { nodeId: 'local', instanceId: 'serving-one', projectBasePath: root, assertAvailable() {}, reviewRegistry: registry };
  const runtime = new LocalGitRuntime(configuration);
  cleanups.push(async () => runtime.dispose());
  return { root, projectPath, runtime, git: runtime.git, original, configuration };
}

async function selection(git: LocalGitRuntime['git'], projectPath: string, file = 'example.txt') {
...
test('local machine service scopes operations and rejects untrusted fields before execution', async () => {
  ...
  expect(await git.getStatus({ projectPath })).toMatchObject({ nodeId: 'local', instanceId: 'serving-one', branch: 'main' });
...
test('rejects an ancestor repository outside the node base and escaping file symlinks', async () => {
  ...
  const restricted = new LocalGitRuntime({ ...configuration, projectBasePath: subdir });
...
test('filters forbidden sibling worktrees and validates missing creation destinations', async () => {
  ...
  const restricted = new LocalGitRuntime({ nodeId: 'local', instanceId: 'scope', projectBasePath: projectPath, assertAvailable() {} });
  cleanups.push(async () => restricted.dispose());
