...
export async function nodeRuntimeFixture(reviewRegistry?: GitReviewDocumentRegistry) {
  ...
  const runtime = new LocalGitRuntime({
    nodeId: 'local', instanceId: 'test-instance', projectBasePath: root,
...
export async function untrackedReview(git: LocalGitRuntime['git'], projectPath: string, file: string) {
