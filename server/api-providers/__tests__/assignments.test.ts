...
test('metadata revision, assignment and provider/endpoint identity are checked at credential release', async () => {
  ...
  const resolver = new ApiProviderEndpointResolver((node) => access.list(node), (_agent, node) => node === remote ? ['openai-compatible'] : [], () => store.list());
  const selection = resolver.resolveSelection({ nodeId: remote, agentId: 'test', model: 'synthetic-model', apiProviderId: provider.id, modelEndpointId: reference.endpointId });
...
test('create keeps an identifiable unassigned profile when node assignment fails', async () => {
  ...
  try {
    ...
    expect(created.assignment).toMatchObject({ nodeId: remote, status: 'not-assigned' });
...
test('missing historical model classification cannot authorize switching a local session to cloud', async () => {
  ...
  const resolver = new ApiProviderEndpointResolver((node) => access.list(node), () => ['openai-compatible'], () => store.list());
