...
export interface SessionSettingsExecutionBarrier {
  runWithAutomaticDispatchSuppressed<T>(
    chatId: string,
    operation: () => Promise<T>,
  ): Promise<T>;
}
...
export class AgentSessionSettingsService {
  ...
  constructor(private readonly deps: {
    ...
    executionBarrier: SessionSettingsExecutionBarrier;
  }) {
  ...
  updateSessionSettings(
    ...
    return this.deps.executionBarrier.runWithAutomaticDispatchSuppressed(
      chatId,
      () => this.#lock.runExclusive(`chat:${chatId}`, async () => {
        ...
        const next = this.deps.endpointResolver.resolveSelection({
          ...
          apiProviderId: patch.apiProviderId !== undefined
            ? patch.apiProviderId
            : entry.apiProviderId,
        ...
        if (endpoint) {
          if (!integration.endpoints) {
            throw new Error(
              `Agent integration ${entry.agentId} does not accept API provider endpoints`,
            );
          }
