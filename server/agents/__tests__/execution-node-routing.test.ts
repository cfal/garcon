...
async function fixture(chatNodes: readonly string[] = IDS) {
  ...
  const nodes = {
    ...
    integrationsFor(nodeId: string) {
      if (!ready.has(nodeId)) throw new DomainError('EXECUTION_NODE_UNAVAILABLE', 'Execution node unavailable', 503, true);
  ...
  for (const [index, nodeId] of chatNodes.entries()) {
    chats.addChat({
...
test('node loss closes every producer even when one chat cannot commit its terminal', async () => {
...
test('a permission reply remains actionable while its node is reconnecting', async () => {
  const f = await fixture();
