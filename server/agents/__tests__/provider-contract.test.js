...
describe('shared agent/API provider contract', () => {
  it('maps Anthropic-compatible endpoints to Claude Code and Direct Anthropic', () => {
    ...
    expect(isAgentCompatibleWithProtocol(DIRECT_ANTHROPIC_COMPATIBLE_AGENT_ID, 'anthropic-messages')).toBe(true);
    expect(isAgentCompatibleWithProtocol('codex', 'anthropic-messages')).toBe(false);
    expect(isAgentCompatibleWithProtocol(DIRECT_OPENAI_CHAT_COMPLETIONS_COMPATIBLE_AGENT_ID, 'anthropic-messages')).toBe(false);
  });
  ...
  it('maps OpenAI-compatible endpoints to broad compatible consumers', () => {
    ...
    expect(isAgentCompatibleWithProtocol(DIRECT_OPENAI_CHAT_COMPLETIONS_COMPATIBLE_AGENT_ID, 'openai-compatible')).toBe(true);
    expect(isAgentCompatibleWithProtocol(DIRECT_OPENAI_RESPONSES_COMPATIBLE_AGENT_ID, 'openai-compatible')).toBe(true);
    expect(isAgentCompatibleWithProtocol('claude', 'openai-compatible')).toBe(false);
    expect(isAgentCompatibleWithProtocol(DIRECT_ANTHROPIC_COMPATIBLE_AGENT_ID, 'openai-compatible')).toBe(false);
  });
  ...
  it('maps OpenAI-compatible endpoint capabilities to agents', () => {
    ...
    expect(endpointSupportsAgent(DIRECT_OPENAI_CHAT_COMPLETIONS_COMPATIBLE_AGENT_ID, {
      protocol: 'openai-compatible',
      capabilities: { chatCompletions: true, responses: false },
    })).toBe(true);
    expect(endpointSupportsAgent(DIRECT_OPENAI_RESPONSES_COMPATIBLE_AGENT_ID, {
      protocol: 'openai-compatible',
      capabilities: { chatCompletions: true, responses: false },
    })).toBe(false);
    expect(endpointSupportsAgent(DIRECT_OPENAI_RESPONSES_COMPATIBLE_AGENT_ID, {
    ...
    expect(agentsForEndpoint({
      protocol: 'openai-compatible',
      capabilities: { chatCompletions: false, responses: true },
    })).toEqual(['codex', DIRECT_OPENAI_RESPONSES_COMPATIBLE_AGENT_ID]);
  });

  it('does not treat API provider ids as visible agent ids', () => {
    expect(isVisibleAgentId('claude')).toBe(true);
    expect(isVisibleAgentId(DIRECT_OPENAI_CHAT_COMPLETIONS_COMPATIBLE_AGENT_ID)).toBe(true);
    expect(isVisibleAgentId(DIRECT_OPENAI_RESPONSES_COMPATIBLE_AGENT_ID)).toBe(true);
    expect(isVisibleAgentId(DIRECT_ANTHROPIC_COMPATIBLE_AGENT_ID)).toBe(true);
  ...
  it('keeps settings auth rows narrower than visible agents', () => {
    ...
    expect(isOAuthAgentId(DIRECT_OPENAI_CHAT_COMPLETIONS_COMPATIBLE_AGENT_ID)).toBe(false);
    expect(isOAuthAgentId(DIRECT_OPENAI_RESPONSES_COMPATIBLE_AGENT_ID)).toBe(false);
    expect(isOAuthAgentId(DIRECT_ANTHROPIC_COMPATIBLE_AGENT_ID)).toBe(false);
    ...
    expect(isOtherSettingsAgentId(DIRECT_OPENAI_CHAT_COMPLETIONS_COMPATIBLE_AGENT_ID)).toBe(false);
    expect(isOtherSettingsAgentId(DIRECT_OPENAI_RESPONSES_COMPATIBLE_AGENT_ID)).toBe(false);
    expect(isOtherSettingsAgentId(DIRECT_ANTHROPIC_COMPATIBLE_AGENT_ID)).toBe(false);
  });

  it('identifies endpoint-only direct agents', () => {
    expect(isEndpointOnlyAgentId(DIRECT_OPENAI_CHAT_COMPLETIONS_COMPATIBLE_AGENT_ID)).toBe(true);
    expect(isEndpointOnlyAgentId(DIRECT_OPENAI_RESPONSES_COMPATIBLE_AGENT_ID)).toBe(true);
    expect(isEndpointOnlyAgentId(DIRECT_ANTHROPIC_COMPATIBLE_AGENT_ID)).toBe(true);
