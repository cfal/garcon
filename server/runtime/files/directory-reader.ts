...
import { isProjectBoundaryError, resolveRealWithinCanonicalBase } from '../../common/path-boundary.js';
import { hasNodeErrorCode } from '../../common/errors.js';
import { toExecutorPath } from '../../common/executor-path.js';
...
export function relativeFilePath(root: string, target: string): string {
  return toExecutorPath(path.relative(root, target));
}

export function fileBreadcrumbs(root: string, target: string): FileTreeBreadcrumb[] {
  const result = [{ name: path.basename(root) || root, path: toExecutorPath(root) }];
  let current = root;
  for (const segment of path.relative(root, target).split(path.sep).filter(Boolean)) {
    current = path.join(current, segment);
    result.push({ name: segment, path: toExecutorPath(current) });
  }
  return result;
}
