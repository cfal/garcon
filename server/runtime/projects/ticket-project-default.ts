...
interface TicketProjectResolverOptions {
  readonly inspect: (projectPath: string) => Promise<ProjectResolution>;
...
export async function resolveTicketProjectDefault(directory: string, signal: AbortSignal | undefined,
  ...
  try {
    ...
    if (bare === 'true\n') return { project: ticketProject(basename(commonDirectory)), kind: 'repository' };
    if (bare !== 'false\n') throw unavailable();
    ...
    if (configuredCheckout) {
      return { project: ticketProject(basename(await canonical(configuredCheckout))), kind: 'repository' };
    }
    const checkout = primaryCheckout(await probe(cwd, ['worktree', 'list', '--porcelain', '-z']));
    if (checkout === commonDirectory) throw unavailable();
    return { project: ticketProject(basename(await canonical(checkout))), kind: 'repository' };
  } catch {
    signal?.throwIfAborted();
    const name = basename(cwd);
    if (!name) throw unavailable();
    return { project: ticketProject(name), kind: 'folder' };
  }
}
