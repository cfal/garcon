...
export class TerminalManager {
  ...
  constructor(options: TerminalManagerOptions) {
    ...
    this.#userShell = options.userShell ?? defaultUserShell();
  ...
  async create(
    ...
    return this.#createLock.runExclusive(principal.key, async () => {
      ...
      if (request.expectedTerminalRuntimeId !== undefined && request.expectedTerminalRuntimeId !== this.terminalRuntimeId) {
        throw new TerminalManagerError('terminal-runtime-changed', 'Terminal executor restarted. Refresh its terminal list before creating a new shell.', 409);
      }
      ...
      this.#assertRunning();
      const terminalId = qualifiedTerminalId({ executorId: this.executorId, terminalRuntimeId: this.terminalRuntimeId, sessionId: crypto.randomUUID() });
      let pty: IPty;
      try {
        const shell = this.#userShell;
        const options = {
          name: "xterm-256color",
          cols: 80,
          rows: 24,
      ...
      const metadata: TerminalMetadata = {
        ...
        displaySequence,
        title: null,
        initialWorkingDirectory: toExecutorPath(cwd),
  ...
  #assertRunning(): void {
    if (this.#stopped) throw new TerminalManagerError('terminal-unavailable', 'Terminal executor is stopping.', 503);
  }
