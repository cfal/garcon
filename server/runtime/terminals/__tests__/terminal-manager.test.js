...
describe("TerminalManager", () => {
  it("uses the injected runtime base and shell without controller configuration", async () => {
    ...
    const manager = new TerminalManager({
      ...
      userShell: '/synthetic/runtime-shell',
      spawnPty: (shell, args, options) => {
        calls.push({ shell, args, cwd: options.cwd });
        return new FakePty();
      },
    });
    try {
      await manager.create(principal('alice'), {
        requestId: 'runtime-options', requestedInitialWorkingDirectory: null,
      });
      expect(calls).toEqual([{
        shell: '/synthetic/runtime-shell', args: [], cwd: await fs.realpath(projectPath),
      }]);
    } finally {
      manager.shutdown();
    }
  ...
  it("creates idempotently, isolates principals, and retains exited sessions", async () => {
    const ptys = [];
    const manager = new TerminalManager({ projectBasePath: projectPath,
      spawnPty: () => {
  ...
  it("renames running and exited sessions and broadcasts the current metadata", async () => {
    const pty = new FakePty();
    const manager = new TerminalManager({ projectBasePath: projectPath, spawnPty: () => pty });
    const alice = principal("alice");
    const bob = principal("bob");
  ...
  it("enforces the principal cap under concurrent creation", async () => {
    const manager = new TerminalManager({ projectBasePath: projectPath, spawnPty: () => new FakePty() });
    const alice = principal("alice");
