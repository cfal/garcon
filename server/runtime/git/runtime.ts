...
import type { ExecutionGitService, ExecutionGhService, ExecutorCallOptions } from '@garcon/server-agent-interface';
import { AgentCallError } from '@garcon/server-agent-interface';
import type { GitMethod } from '../../../common/git.js';
import { GIT_MAX_CONCURRENT_QUERIES, isGitMutation, type ExecutionGitRequests, type ExecutionGitResults, type ExecutionGhResults, type GitExecutorScope } from '../../../common/git-execution.js';
...
interface GitExecutorOptions extends GitExecutorScope {
...
export class GitRuntime {
  ...
  constructor(private readonly configuration: GitExecutorOptions) {
  ...
  async #repository(projectPath: string, options?: ExecutorCallOptions): Promise<string> {
  ...
  async #run<K extends GitMethod>(method: K, request: ExecutionGitRequests[K], options?: ExecutorCallOptions): Promise<ExecutionGitResults[K]> {
    ...
    try {
      return await this.#admit(isGitMutation(method), options, async (callOptions) => {
        ...
        const execute = async () => {
          ...
          if ('document' in input) {
            if (input.document.executorId !== this.configuration.executorId || input.document.instanceId !== this.configuration.instanceId) throw new GitServiceError('GIT_STALE_DOCUMENT', 'Git review belongs to a different executor or serving instance');
          ...
          const response = { ...portableResult(result), executorId: this.configuration.executorId, instanceId: this.configuration.instanceId, diagnostics,
      ...
      if (failure instanceof GitServiceError && failure.code === 'GIT_NOT_REPO') {
        ...
        if (result) return { ...result, executorId: this.configuration.executorId, instanceId: this.configuration.instanceId } as ExecutionGitResults[K];
      }
      throw failure;
    }
  }
  ...
  async #ghCall<K extends keyof ExecutionGhResults>(method: K, request: { projectPath?: string; number?: number }, options?: ExecutorCallOptions): Promise<ExecutionGhResults[K]> {
    ...
    try {
      ...
      const result = { ...payload, executorId: this.configuration.executorId, instanceId: this.configuration.instanceId };
...
function portableResult<T>(result: T): T {
  ...
  const convert = (value: Record<string, unknown>, keys: readonly string[]) => {
    for (const key of keys) if (typeof value[key] === 'string') value[key] = toExecutorPath(value[key]);
  };
