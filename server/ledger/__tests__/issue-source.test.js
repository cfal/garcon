...
import { ISSUE_OUTCOME_QUERY_SQL } from '../issue-outcome-query.js';
...
beforeEach(() => {
  directory = mkdtempSync(join(homedir(), 'garcon-issue-source-'));
...
test('finds only exact correlation, deterministically, without scanning or changing source facts', async () => {
  store.initializeCurrentView(CHAT, { viewId: VIEW, contentStartOrdinal: 1, rows: [
    { kind: 'notice', at: AT, providerMeta: null, message: 'Synthetic private request', detail: {
      type: 'issue-command-request', command: { type: 'issue', ref: 'synthetic',
        payload: { action: 'create', input: { title: 'Synthetic issue' } } },
    } }, notice(1, OTHER), notice(2), notice(), notice(),
  ] });
  expect(await reader.resolveIssueSource(source)).toEqual({ kind: 'found', target: { ...source, ordinal: 4 } });
  expect(source.ordinal).toBe(1);
  expect(await reader.resolveIssueSource({ ...source, ordinal: 99 }))
    .toEqual({ kind: 'outcome-unavailable', chatId: CHAT });
  const db = new Database(join(directory, CHAT, 'ledger.sqlite'));
  try {
    const plan = db.query(`EXPLAIN QUERY PLAN ${ISSUE_OUTCOME_QUERY_SQL}`).all(VIEW, VIEW, 1);
    expect(plan.map((row) => row.detail).join(' ')).toContain('USING INDEX transcript_issue_outcome_correlation');
...
test('migrates schema v1 transactionally on lazy open without rewriting row addresses', async () => {
  ...
  const db = new Database(join(directory, CHAT, 'ledger.sqlite'));
  db.exec('DROP INDEX transcript_issue_outcome_correlation; PRAGMA user_version = 1');
  db.close();
  expect(await reader.resolveIssueSource(source)).toEqual({ kind: 'found', target: source });
...
test('never recovers old correlation from an imported replacement view', async () => {
  ...
  const lookup = spyOn(store, 'issueOutcomeOrdinal');
  try {
    expect(await reader.resolveIssueSource(source)).toEqual({ kind: 'transcript-reloaded', chatId: CHAT });
    expect(lookup).not.toHaveBeenCalled();
    expect(() => store.issueOutcomeOrdinal(CHAT, VIEW, 1)).toThrow(StaleTranscriptViewError);
  } finally { lookup.mockRestore(); }
});

test('absent ledgers are not created, cancellation propagates, corruption is not missing', async () => {
  expect(await reader.resolveIssueSource(source)).toEqual({ kind: 'outcome-unavailable', chatId: CHAT });
  expect(existsSync(join(directory, CHAT))).toBe(false);
  const abort = new AbortController();
  abort.abort(new Error('Synthetic cancellation'));
  await expect(reader.resolveIssueSource(source, abort.signal)).rejects.toThrow('Synthetic cancellation');
  store.initializeCurrentView(CHAT, { viewId: VIEW, contentStartOrdinal: 1 });
  store.closeChat(CHAT);
  const db = new Database(join(directory, CHAT, 'ledger.sqlite'));
  db.exec('PRAGMA user_version = 999');
  db.close();
  await expect(reader.resolveIssueSource(source)).rejects.toBeInstanceOf(TranscriptHistoryUnavailableError);
});
