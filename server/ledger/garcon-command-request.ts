...
export function agentActionRequestNoticeDraft(at: string, command: GarconStartAgentCommand | GarconScheduleCommand): LedgerRowDraft {
  return {
    kind: 'notice', at,
    message: command.type === 'start-agent' ? 'Agent requested child creation' : 'Agent requested prompt scheduling',
    detail: {
      type: command.type === 'start-agent' ? AGENT_START_REQUEST_NOTICE_TYPE : AGENT_SCHEDULE_REQUEST_NOTICE_TYPE,
      command: command.type === 'start-agent' ? { ...command } : { ...command, firstRun: { ...command.firstRun } },
    },
    providerMeta: null,
  };
}
