...
export const TICKET_COMMAND_REQUEST_NOTICE_TYPE = 'ticket-command-request';

export function ticketCommandRequestNoticeDraft(at: string, command: GarconTicketCommand): LedgerRowDraft {
  return {
    kind: 'notice', at, message: 'Agent requested ticket management',
    detail: { type: TICKET_COMMAND_REQUEST_NOTICE_TYPE, command: JSON.parse(JSON.stringify(command)) as JsonObject },
    providerMeta: null,
  };
}
...
export function isLedgerPrivateGarconCommandRow(row: LedgerRow): boolean {
  return row.kind === 'notice'
    && (
      ...
      || row.detail.type === TICKET_COMMAND_REQUEST_NOTICE_TYPE
    );
}
