...
import { InProcessExecutionNode } from '../in-process.ts';
import { TerminalRuntime } from '../../terminals/node-service.ts';
import { RemoteExecutionNode } from '../remote.ts';
import { WebSocketLink } from '../websocket-link.ts';
import { serveAgentNode } from '../agent-worker.ts';
...
for (const dialer of ['controller', 'worker']) test(`terminal RPC preserves process lifetime with ${dialer} dialing`, async () => {
  const options = { nodeId: crypto.randomUUID(), secret: Buffer.alloc(32, 42).toString('base64url'), allowInsecureDevelopment: true, reconnectDelayMs: 10 };
  const ptys = [];
  ...
  const scopes = [];
  worker.onSession(transport => scopes.push(serveAgentNode(new InProcessExecutionNode({
    id: options.nodeId, workspaceDir: homedir(), projectBasePath: homedir(), integrations: [], terminalRuntime: runtime, resolveCredential: async () => null,
  }), new AgentRpc(transport))));
  ...
  try {
    ...
    const request = { nodeId: options.nodeId, expectedTerminalRuntimeId: inventory.terminalRuntimeId, requestId: 'create', requestedInitialWorkingDirectory: null };
    ...
    const before = await node.getInfo();
    controller.current.close(new Error('Replace logical session'));
    await until(() => node.availability === 'ready' && scopes.length === 2);
    const nextInventory = await service.list(authority);
    expect((await node.getInfo()).instanceId).not.toBe(before.instanceId);
