...
async function fixture() {
  ...
  const root = await mkdtemp(join(temporary, 'node-projects-'));
  roots.push(root);
  const base = join(root, 'worker');
  const project = join(base, 'project');
  await mkdir(project, { recursive: true });
  let disposed = false;
  const service = new LocalExecutionProjectService(base, (options) => {
...
test('a symlinked configured base accepts its own canonical project paths', async () => {
  ...
  const service = new LocalExecutionProjectService(alias, () => {});
...
test('portable path conversion preserves roots, spaces, Unicode, and POSIX backslashes', () => {
  ...
  for (const native of windows) {
    const portable = toNodePath(native, '\\');
  ...
  for (const native of ['/', '/home/project with space', '/home/\u03bb', '/home/literal\\name']) {
    expect(toNodePath(native, '/')).toBe(native);
    expect(toNativePath(native, '/')).toBe(native);
  }
  expect(toNodePath('\\\\server\\share\\', '\\')).toBe('//server/share/');
  expect(isWithinNodePath('C:/', 'C:/project')).toBe(true);
  expect(isWithinNodePath('//server/share', '//server/shared/project')).toBe(false);
});
...
for (const dialer of ['controller', 'worker'] as const) {
  test(`project service uses the current worker session without local fallback (${dialer} dials)`, async () => {
    ...
    try {
      const projects = await remote.node.getProjectService();
      expect((await remote.node.getInfo()).projectBasePath).toBe(f.base);
