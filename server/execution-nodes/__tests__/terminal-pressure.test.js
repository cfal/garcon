...
for (const closeBeforeReply of [false, true]) test(`attachment cleanup bypasses RPC saturation (pending attach: ${closeBeforeReply})`, async () => {
  ...
  controllerSocket = controller.attach({ send(data) { queueMicrotask(() => nodeSocket.receive(data)); }, close() {} });
  nodeSocket = node.attach({ send(data) { queueMicrotask(() => controllerSocket.receive(data)); }, close() {} });
