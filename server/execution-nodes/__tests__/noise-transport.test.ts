...
const options = { secret, nodeId: 'synthetic-node', allowInsecureDevelopment: true, reconnectDelayMs: 20 };
...
test.each(['key', 'context'])('wrong Noise %s is rejected before the Garcon hello or a logical session', async (wrong) => {
  ...
  const socket = connectNoiseWebSocket(worker.listen(), {
    psk: Buffer.alloc(32, wrong === 'key' ? 43 : 42), context: wrong === 'context' ? 'wrong-context' : EXECUTION_NODE_NOISE_CONTEXT,
