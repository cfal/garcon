...
test.each(['controller', 'worker'] as const)('dispatches endpoint discovery on the worker when %s dials', async (dialer) => {
  ...
  fixture.generations[0]!.node.discoverApiProviderModels = discover;
  try {
    expect(await fixture.node.discoverApiProviderModels(request)).toEqual({ success: true, models });
    expect(discover).toHaveBeenCalledWith(request, { signal: expect.any(AbortSignal) });
    await fixture.node.dispose();
    const failure = await fixture.node.discoverApiProviderModels(request).catch((error) => error);
