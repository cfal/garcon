...
async function fixture(routes: RouteMap = {}, workspaceName: string | null = 'workspace') {
  ...
  const discover = () => discoverRuntime({ configDir: root, runtime: 'execution-node' });
...
test('private role descriptors prove a live endpoint and never expose the controller capability or directory', async () => {
  ...
  const second = await startCliGateway({ dataDir: join(secondRoot, 'execution-node'), currentRpc: () => null });
  try {
    ...
    await expect(discoverRuntime({ configDir: secondRoot, runtime: 'execution-node' })).rejects.toThrow('context unavailable');
