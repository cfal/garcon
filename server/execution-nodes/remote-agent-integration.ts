...
import type { AgentRpcMethods, IntegrationManifest } from './agent-protocol.js';
...
export class RemoteAgentIntegration implements AgentIntegration {
  ...
  constructor(readonly manifest: IntegrationManifest, current: () => RemoteSessionBacking) {
    ...
    const call = async <K extends keyof AgentRpcMethods>(method: K, request: AgentRpcMethods[K]['request'], options?: Parameters<AgentRpc['call']>[3]) => (
