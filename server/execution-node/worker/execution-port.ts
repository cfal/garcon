...
import {
  parseNodeExecutionCallText, parseNodeExecutionCancellationText,
...
import type { NodeFrameSubmission, NodeFrameWriter, NodeWorkerFramePriority } from './writer.js';
...
export class NodeWorkerExecutionPort implements Pick<NodeSocketWriter, 'send' | 'close'> {
  ...
  send(payload: string): boolean {
    ...
    if (cancel) {
      ...
      return this.#send(payload, null, 'urgent');
    }
    const request = parseNodeExecutionCallText(payload);
    if (!request) return this.#send(payload, null);
    ...
    try { return this.#send(payload, { requestId: request.requestId, pending }); }
  ...
  #send(payload: string, request: { requestId: number; pending: PendingWrite } | null, priority: NodeWorkerFramePriority = 'data'): boolean {
    ...
    try { submission = this.writer.submit(text, priority, { signal, validate: () => this.#validate() }); }
    catch (error) {
    ...
    if (pending) {
      ...
      if (pending.cancelled && submission.submitted) this.#send(pending.cancelled, null, 'urgent');
    }
