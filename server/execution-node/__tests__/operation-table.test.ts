...
import type { AgentExecutionHandle, AgentStartRequestV5, AgentResumeRequestV5, AgentProducerEvent, AgentSteerRequest, AgentSteerResult, AgentGoalControlRequest } from '@garcon/server-agent-interface';
...
import { NodeOperationTable, type NodeExecutionRequest, type NodeControlPreparation } from '../operation-table.js';
...
function fixture(facets: Partial<Pick<ConstructorParameters<typeof LocalProviderExecutionService>[0], 'steering' | 'goals'>> = {}) {
  let elapsed = 0;
  ...
  const integration = {
    ...
    execution, compaction: null, steering: null, goals: null, ...facets,
  ...
  const table = new NodeOperationTable({ supervisor, connection, resources,
    limits: { maxOperations: 1, preparationMs: 100, receiptMs: 100, maxReceipts: 1 }, scheduleTimeout: () => ({ cancel() {} }) });
  ...
  return { caller, supervisor, connection, identity, execution, service, resources, configuration, table, location, request, input, output, events,
...
test('an abort awaits its own late handle and retains capacity until the matching terminal', async () => {
  const f = fixture();
  const ready = Promise.withResolvers<AgentExecutionHandle>();
  f.execution.start.mockImplementation(async () => ready.promise);
  const ticket = await f.prepare();
...
test('physical recovery allows status and abort without authorizing another dispatch', async () => {
  ...
  expect(f.table.status(replacement, ticket.identity)).toMatchObject({ kind: 'completed', value: { dispatch: 'completed' } });
...
test('a matching terminal before handle return releases capacity when dispatch settles', async () => {
  const f = fixture();
  ...
  const ticket = await f.prepare();
  await f.table.dispatch(f.connection, ticket.identity, f.input, f.output);
  expect(f.table.status(f.connection, ticket.identity)).toMatchObject({ kind: 'completed', value: { phase: 'ended' } });
  await expect(f.prepare()).resolves.toMatchObject({ runId: f.request.runId });
});

test('a definite local dispatch failure releases capacity without claiming process termination', async () => {
  const f = fixture();
  f.execution.start.mockImplementation(async () => { throw new Error('synthetic launch failure'); });
  const ticket = await f.prepare();
  await expect(f.table.dispatch(f.connection, ticket.identity, f.input, f.output)).rejects.toThrow('synthetic launch failure');
  expect(f.table.status(f.connection, ticket.identity)).toMatchObject({
    kind: 'completed', value: { phase: 'failed', dispatch: 'failed', abort: null },
  });
  await expect(f.prepare()).resolves.toMatchObject({ runId: f.request.runId });
  expect(f.execution.abort).not.toHaveBeenCalled();
});
...
test('a failed terminal publication aborts the exact handle without failing native launch', async () => {
  ...
  expect(f.table.status(f.connection, ticket.identity)).toMatchObject({
    value: { phase: 'failed', dispatch: 'completed', abort: 'pending' },
  });
  await expect(f.prepare()).rejects.toMatchObject({ code: 'NODE_CAPACITY' });
  abort.resolve(true);
...
test('a lost physical dispatch reply reconciles the original operation without starting another', async () => {
  ...
  expect(f.table.status(replacement, ticket.identity)).toMatchObject({
    kind: 'completed', value: { phase: 'dispatched', dispatch: 'completed' },
  });
...
test('an already revoked resource cannot enter provider preparation', async () => {
  ...
  const table = new NodeOperationTable({ connection: f.connection, supervisor: f.supervisor,
    resources: { prepare: async () => ({ ...resource, signal: revoked.signal, execution: { ...f.service,
      prepare, dispatch: f.service.dispatch.bind(f.service), release: f.service.release.bind(f.service), abort: f.service.abort.bind(f.service),
...
test.each(['prepare', 'status'] as const)('authority expiry inside table polling fences %s', async (action) => {
  ...
  const table = new NodeOperationTable({ connection: f.connection, resources,
    supervisor: {
...
test('close attempts every unused ticket release even when each provider release throws', async () => {
  const f = fixture();
  const table = new NodeOperationTable({ connection: f.connection, supervisor: f.supervisor, resources: f.resources,
    limits: { maxOperations: 2 }, scheduleTimeout: () => ({ cancel() {} }),
  });
  await table.prepare(f.connection, f.location, f.request, f.caller.signal);
  await table.prepare(f.connection, f.location, { ...f.request, runId: 'synthetic-second-run' }, f.caller.signal);
